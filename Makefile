...
ifeq ($(strip $(MSTC_TE_ATTE)),y)
	...
	cp $(HOSTTOOLS_DIR)/createNandImage $(SMTBAK)/createNandImage
endif
...
ifeq ($(strip $(MSTC_TE_ATTE)),y)
	...
	if [ "$(MSTC_NAND_BLOCK_16)" = "y" ]; then \
		echo ./createNandImage -V \"$(MSTC_EXTERNAL_VERSION)\" -Q 1 -b 4 -e 4 -P 512 -m 13 -t 0 -O \"$(MSTC_NAND_FLASH_SIZE)\" -B 16 -G 20 -M 1 >> $(SMTBAK)/GenSMT.sh; \
		echo ./createNandImage -i $(FLASH_IMAGE_NAME)_NAND128_TE -V \"$(MSTC_EXTERNAL_VERSION)\" -v \"$(shell date '+%y%m%d_%H%M')\" -Q 1 -B 16 -O \"$(MSTC_NAND_FLASH_SIZE)\" -b 4 -e 4 -P 512 -m 13 -t 0 -G 20 -M 0 >> $(SMTBAK)/GenSMT.sh; \
	else \
		echo ./createNandImage -V \"$(MSTC_EXTERNAL_VERSION)\" -Q 1 -b 4 -e 4 -P 2048 -m 13 -t 0 -O \"$(MSTC_NAND_FLASH_SIZE)\" -B 128 -G 20 -M 1 >> $(SMTBAK)/GenSMT.sh; \
		echo ./createNandImage -i $(FLASH_IMAGE_NAME)_NAND128_TE -V \"$(MSTC_EXTERNAL_VERSION)\" -v \"$(shell date '+%y%m%d_%H%M')\" -Q 1 -B 128 -O \"$(MSTC_NAND_FLASH_SIZE)\" -b 4 -e 4 -P 2048 -m 13 -t 0 -G 20 -M 0 >> $(SMTBAK)/GenSMT.sh; \
	fi
	...
	echo "echo ********************Generate SMT file for 4 bit ECC END********************" >> $(SMTBAK)/GenSMT.sh
	echo "echo ********************Compare SMT file for 4 bit ECC Begin********************" >> $(SMTBAK)/GenSMT.sh
	...
	echo "echo ********************Compare SMT file for 4 bit ECC END********************" >> $(SMTBAK)/GenSMT.sh

	echo rm common_header.img partition_header.img product_header.img partition_image.img >> $(SMTBAK)/GenSMT.sh
	if [ "$(MSTC_NAND_BLOCK_16)" = "y" ]; then \
	...
	echo cat common_header.img partition_header.img \> product_header.img >> $(SMTBAK)/GenSMT.sh
	echo cat product_header.img partition_image.img \> $(SMT_IMAGE_NAME)_NAND128_1_ECC >> $(SMTBAK)/GenSMT.sh
	echo cat partition_image.img \> $(SMT_IMAGE_NAME)_NAND128_1_ECC_partition_image >> $(SMTBAK)/GenSMT.sh	
	echo "echo ********************Generate SMT file for 1 bit ECC END********************" >> $(SMTBAK)/GenSMT.sh
	echo "echo ********************Compare SMT file for 1 bit ECC Begin********************" >> $(SMTBAK)/GenSMT.sh
	echo if [ \"$$\(diff ./$(SMT_IMAGE_NAME)_NAND128_1_ECC ../$(SMT_IMAGE_NAME)_NAND128_1_ECC\)\" = \"\" ]\; then  >> $(SMTBAK)/GenSMT.sh
	echo "echo -e \"\033[32mThe SMT file ./$(SMT_IMAGE_NAME)_NAND128_1_ECC generated by GenSMT.sh is same as ../$(SMT_IMAGE_NAME)_NAND128_1_ECC\033[0m\"" >> $(SMTBAK)/GenSMT.sh
	echo "else" >> $(SMTBAK)/GenSMT.sh
	echo "echo -e \"\033[33mThe SMT file ./$(SMT_IMAGE_NAME)_NAND128_1_ECC generated by GenSMT.sh is different as ../$(SMT_IMAGE_NAME)_NAND128_1_ECC\033[0m\"" >> $(SMTBAK)/GenSMT.sh
	echo "fi" >> $(SMTBAK)/GenSMT.sh
	echo "echo ********************Compare SMT file for 1 bit ECC END********************" >> $(SMTBAK)/GenSMT.sh
...
buildimage: SMT_BAK kernelbuild libcreduction gen_credits
...
# star __MSTC__, RaynorChung: Support 963268, patch form SVN#3597 on http://svn.zyxel.com.tw/svn/CPE_SW1/BCM96368/trunk/P-870HA/branches/cht/fttb8/4.11 
	...
	rm file128k.tmp cferom.tmp; \
      ...
      if [ "$(MSTC_NAND_BLOCK_16)" = "y" ]; then \
         $(HOSTTOOLS_DIR)/createNandImage -V $(MSTC_EXTERNAL_VERSION) -Q 1 -b 4 -e 4 -P 512 -m 13 -t 0 -O $(MSTC_NAND_FLASH_SIZE) -B 16 -G 20 -M 1; \
         $(HOSTTOOLS_DIR)/createNandImage -i $(FLASH_IMAGE_NAME)_NAND128_TE -V $(MSTC_EXTERNAL_VERSION) -v $(shell date '+%y%m%d_%H%M') -Q 1 -B 16 -O $(MSTC_NAND_FLASH_SIZE) -b 4 -e 4 -P 512 -m 13 -t 0 -G 20 -M 0; \
      else \
         $(HOSTTOOLS_DIR)/createNandImage -V $(MSTC_EXTERNAL_VERSION) -Q 1 -b 4 -e 4 -P 2048 -m 13 -t 0 -O $(MSTC_NAND_FLASH_SIZE) -B 128 -G 20 -M 1; \
         $(HOSTTOOLS_DIR)/createNandImage -i $(FLASH_IMAGE_NAME)_NAND128_TE -V $(MSTC_EXTERNAL_VERSION) -v $(shell date '+%y%m%d_%H%M') -Q 1 -B 128 -O $(MSTC_NAND_FLASH_SIZE) -b 4 -e 4 -P 2048 -m 13 -t 0 -G 20 -M 0; \
      fi ; \
      ...
      cat common_header.img partition_header.img > product_header.img; \
      cat product_header.img partition_image.img > $(SMT_IMAGE_NAME)_NAND128_1_ECC; \
      rm common_header.img partition_header.img product_header.img partition_image.img; \
	...
	if [ "$(BUILD_HPNA)" = "y" ] && [ "$(MSTC_HPNA_SWVERSION)" != """" ]; then \
		if [ "$(MSTC_OBM_IMAGE_DEFAULT)" = "y" ]; then \
			...
			if [ "$(MSTC_ROM_D)" = "y" ]; then \
				...
				$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(FS_KERNEL_IMAGE_NAME)_NAND128 --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(MSTC_INTERNAL_VERSION) --externalversion=$(MSTC_EXTERNAL_VERSION) --modelid=$(MSTC_MODEL_ID) --cmodelid=$(MSTC_CUSTOM_MODEL_ID) --cfefile $(CFE_NAND128_FILE) --rootfsfile rootfs128kb.img --imgdefaultfile=$(MSTC_OBM_IMAGE_DEFAULT_FILE); \
				$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(CFE_FS_KERNEL_IMAGE_NAME)_NAND128 --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(MSTC_INTERNAL_VERSION) --externalversion=$(MSTC_EXTERNAL_VERSION) --modelid=$(MSTC_MODEL_ID) --cmodelid=$(MSTC_CUSTOM_MODEL_ID) --cfefile $(CFE_NAND128_FILE) --rootfsfile rootfs128kb.img --include-cfe --imgdefaultfile=$(MSTC_OBM_IMAGE_DEFAULT_FILE); \
				$(HOSTTOOLS_DIR)/createZyNandimg --boardid=$(BRCM_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --countrycode=$(MSTC_COUNTRY_CODE) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --backuppsi="$(BUILD_BACKUP_PSI)" --inputfile=$(CFE_FS_KERNEL_IMAGE_NAME)_NAND128 --outputfile=$(FLASH_IMAGE_NAME)_NAND128; \
				$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(CFE_FS_KERNEL_IMAGE_NAME)_NAND128_TE --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(MSTC_INTERNAL_VERSION) --externalversion=$(MSTC_EXTERNAL_VERSION) --modelid=$(MSTC_MODEL_ID) --cmodelid=$(MSTC_CUSTOM_MODEL_ID) --cfefile $(CFE_NAND128_FILE_TE) --rootfsfile rootfs128kb.img --include-cfe --imgdefaultfile=$(MSTC_OBM_IMAGE_DEFAULT_FILE); \
				$(HOSTTOOLS_DIR)/createZyNandimg --boardid=$(BRCM_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --countrycode=$(MSTC_COUNTRY_CODE) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --backuppsi="$(BUILD_BACKUP_PSI)" --inputfile=$(CFE_FS_KERNEL_IMAGE_NAME)_NAND128_TE --outputfile=$(FLASH_IMAGE_NAME)_NAND128_TE; \
