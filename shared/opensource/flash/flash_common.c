...
void flash_init_nand_info(NVRAM_DATA *nvRam, FLASH_ADDR_INFO *fInfo)
{
   ...
   int change = 0;
    ...
    unsigned long ulBootBlkNum = 0;
    unsigned long ulRootfsBlkNum = 0;
    unsigned long ulDataBlkNum = 0;
    unsigned long ulFwBlkNum = 0;
    unsigned long ulBbtBlkNum = 0;
    ...
    if (totalSize <= FLASH_LENGTH_BOOT_ROM) {
        ...
        /* This is a boolean field for NAND flash. */
        #if (defined(SUPPORT_BACKUP_PSI) && defined(MLD_GLOB_TTNET_FEATURE))
        fInfo->flash_backup_psi_number_blk = 1; /*Frank*/
        #else
        fInfo->flash_backup_psi_number_blk = nvRam->backupPsi;
        #endif
    ...
    #if 1 /* MSTC_FWUP_FROM_FLASH */
    ulBootBlkNum = (nvramData.ulNandPartSizeKb[NP_BOOT] / ulBlockSizeKb);
    ulBbtBlkNum = (nvramData.ulNandPartSizeKb[NP_BBT] / ulBlockSizeKb);
    ulDataBlkNum = (NAND_DATA_SIZE_KB / ulBlockSizeKb);
    ulRootfsBlkNum = ((totalBlks - (ulBootBlkNum + ulBbtBlkNum + ulDataBlkNum))/3);
    ulFwBlkNum = (totalBlks - (ulBootBlkNum + ulBbtBlkNum + ulDataBlkNum + (2 * ulRootfsBlkNum)));

    /* Add firmware upgrade partition */
    /* The FwUp partition is before the Data and is a constant size. */
    nvramData.ulNandPartOfsKb[NP_FWUPGRADE] = nvramData.ulNandPartOfsKb[NP_BBT] - (ulFwBlkNum * ulBlockSizeKb);
    nvramData.ulNandPartSizeKb[NP_FWUPGRADE] = (ulFwBlkNum * ulBlockSizeKb);

    fInfo->flash_fwupgrade_start_blk = flash_get_blk(FLASH_BASE + nvramData.ulNandPartOfsKb[NP_FWUPGRADE] * 1024);

    /* The Data partition is before the BBT and is a constant size. */
    nvramData.ulNandPartOfsKb[NP_DATA] = nvramData.ulNandPartOfsKb[NP_FWUPGRADE] - NAND_DATA_SIZE_KB;
...
#if 1 /* MSTC_FWUP_FROM_FLASH */
    /* Add firmware upgrade partition */
    fInfo->flash_rootfs_max_size = (ulRootfsBlkNum * flash_get_sector_size(0));
...
//#if 1 //__MSTC__, Dennis
    ...
    nvramData.ulNandPartOfsKb[NP_ROOTFS_2] = nvramData.ulNandPartOfsKb[NP_ROOTFS_1] + nvramData.ulNandPartSizeKb[NP_ROOTFS_1];
    fInfo->flash_rootfs2_start_offset = (nvramData.ulNandPartOfsKb[NP_ROOTFS_2] * 1024);
#else
...
#if 1 /* MSTC_FWUP_FROM_FLASH*/
   /* Add firmware upgrade partition */
   if(nvramData.ulNandPartOfsKb[NP_FWUPGRADE] != nvRam->ulNandPartOfsKb[NP_FWUPGRADE])
   {
      nvRam->ulNandPartOfsKb[NP_FWUPGRADE] = nvramData.ulNandPartOfsKb[NP_FWUPGRADE];
      change++;
   }
   if(nvramData.ulNandPartSizeKb[NP_FWUPGRADE] != nvRam->ulNandPartSizeKb[NP_FWUPGRADE])
   {
      nvRam->ulNandPartSizeKb[NP_FWUPGRADE] = nvramData.ulNandPartSizeKb[NP_FWUPGRADE];
      change++;
   }
#endif
...
#if 1 /* MSTC_FWUP_FROM_FLASH*/
    /* Add firmware upgrade partition */
    printk("fw	offset=0x%8.8lx, size=0x%8.8lx\n",
        nvRam->ulNandPartOfsKb[NP_FWUPGRADE] * 1024,
        nvRam->ulNandPartSizeKb[NP_FWUPGRADE] * 1024);
#endif
...
#if 1 /* MSTC_FWUP_FROM_FLASH*/
    /* Add firmware upgrade partition */
    printk("fInfo->flash_fwupgrade_start_blk = %d\n", fInfo->flash_fwupgrade_start_blk);
    printk("fInfo->flash_rootfs2_start_offset = 0x%x\n", fInfo->flash_rootfs2_start_offset);
    #endif
#endif
