...
/* for 1 bit ECC */
#define MXIC_MX30LF1G08AA_FULL  0xf1801d
/* for 4 bit ECC */
#define MXIC_MX30LF1G18AC_FULL  0xf18095

/* TOSHIBA flash parts. */
#define TOSHIBA_TC58NVG0S3E     0xd1
#define TOSHIBA_TC58NVM9S3E		0xf0
#if 1 /* Support Toshiba TC58BVG0S3HTA00 NAND flash. MitraStar Delon Yu, 20141029. */
#define TOSHIBA_TC58BVG0S3HTA00	0xf1
#endif

#define TOSHIBA_TC58NVG0S3HTAI0 0xf1
...
#define NAND_FLASH_DEVICES                                                    \
  {{NAND_MAKE_ID(FLASHTYPE_SAMSUNG,SAMSUNG_K9F5608U0A),"Samsung K9F5608U0"},  \
   ...
   {NAND_MAKE_ID(FLASHTYPE_TOSHIBA,TOSHIBA_TC58BVG0S3HTA00),"Toshiba TC58BVG0S3HTA00"},\
   {NAND_MAKE_ID(FLASHTYPE_TOSHIBA,TOSHIBA_TC58NVG0S3HTAI0),"Toshiba TC58NVG0S3HTAI0"},\
   {0,""}                                                                     \
  }

#define NAND_MAKE_ID_FULL(A,B)    \
    (((unsigned long) (A) << 24) | ((unsigned long) B & 0xffffff))

#define NAND_FLASH_DEVICES_FULL                                                    \
  {{NAND_MAKE_ID_FULL(FLASHTYPE_MXIC,MXIC_MX30LF1G08AA_FULL),"MXIC MX30LF1G08AA"},      \
   {NAND_MAKE_ID_FULL(FLASHTYPE_MXIC,MXIC_MX30LF1G18AC_FULL),"MXIC MX30LF1G18AC"},      \
   {0,""}                                                                     \
  }
...
struct flash_name_from_id_FULL
{
    unsigned long fnfi_id;
    char fnfi_name[30];
};
...
#if 1// Force erase block even it contains non 0xFF value in BI, DelonYu, 20150907
static int nandflash_block_erase_force(PCFE_NAND_CHIP pchip, unsigned long blk_addr );
#endif
#if 1//__MSTC__, RaynorChung: Support 963268 nand flash, patch form SVN#3597 on http://svn.zyxel.com.tw/svn/CPE_SW1/BCM96368/trunk/P-870HA/branches/cht/fttb8/4.11 
...
int nand_flash_init(flash_device_info_t **flash_info)
{
    ...
    if( initialized == 0 )
    {
        ...
        static struct flash_name_from_id_FULL fnfi_FULL[] = NAND_FLASH_DEVICES_FULL;
        struct flash_name_from_id_FULL *fnfi_ptr_FULL;
        ...
        for( fnfi_ptr_FULL= fnfi_FULL; fnfi_ptr_FULL->fnfi_id != 0; fnfi_ptr_FULL++ )
        {
            if( fnfi_ptr_FULL->fnfi_id == pchip->chip_device_id )
            {
                strcpy(flash_nand_dev.flash_device_name, fnfi_ptr_FULL->fnfi_name);
                break;
            }
        }
...
static void nand_read_cfg(PCFE_NAND_CHIP pchip)
{
    ...
    else
    {
        ...
        if( ecc_lvl == NAC_ECC_LVL_DISABLE )
            printk("NAND ECC Disabled, ");
        else if( ecc_lvl == NAC_ECC_LVL_HAMMING )
            printk("NAND ECC Hamming, ");
        else
            printk("NAND ECC BCH-%d, ", ecc_lvl);
        printk("page size 0x%x bytes, spare size used %d bytes\n", pchip->chip_page_size, pchip->chip_spare_size);
...
#if 1 /* Michael add for bad block test */
void mark_bad_blk(unsigned short blk);
void mark_bad_blk(unsigned short blk)
{
    PCFE_NAND_CHIP pchip = &g_chip;
    static int marking_bad_blk = 0;
...
void mark_good_blk(unsigned short blk);
void mark_good_blk(unsigned short blk)
{
    PCFE_NAND_CHIP pchip = &g_chip;
    static int marking_bad_blk = 0;
    ...
    if( marking_bad_blk == 0 )
    {
        ...
        DBG_PRINTF(">> nand_mark_bad_blk - addr=0x%8.8lx, block=0x%8.8lx\n",
            page_addr, page_addr / pchip->chip_block_size);
#if 1 // Force erase block even it contains non 0xFF value in BI, DelonYu, 20150907
        nandflash_block_erase_force(pchip, page_addr);
#else
        nandflash_block_erase(pchip, page_addr);
#endif
        memset(spare, 0xff, pchip->chip_spare_size);
...
static int nandflash_check_ecc(void)
{
    ...
    {
#if 0	//__MSTC__, Hill, enhance boot up time for TE test
#if 1//__MSTC__, RaynorChung: Support 963268 nand flash, patch form SVN#3597 on http://svn.zyxel.com.tw/svn/CPE_SW1/BCM96368/trunk/P-870HA/branches/cht/fttb8/4.11 
...
static int nandflash_block_erase_force(PCFE_NAND_CHIP pchip, unsigned long blk_addr )
{
    ...
    DBG_PRINTF(">> nandflash_block_erase_for_bad_block - addr=0x%8.8lx, ret=%d\n", blk_addr,
        ret);

    return( ret );
} /* nandflash_block_erase_for_bad_block */
#endif
...
int raw_nand_flash_write_buf(unsigned short blk, int offset,
    unsigned char *buffer, int numbytes);
static int raw_nandflash_write_page(PCFE_NAND_CHIP pchip, unsigned long page_addr,
    ...
    unsigned char *buffer, int len)
{
    ...
    //printf(">> nand_flash_write_buf - 1 blk=0x%8.8lx, offset=%d, len=%d\n",
      //  blk, offset, len);
	...
	/*Toshiba TC58NVG0S3HTA00 8-bit raw NAND flash:
	Spare area = 128 bytes
	actual OOB size for 8-bit = 64 bytes
	OOB padding size = 128 - 64 = 64 bytes*/
	if(NAND_CHIPID(pchip) == NAND_MAKE_ID(FLASHTYPE_TOSHIBA,TOSHIBA_TC58NVG0S3HTAI0))
		spare_size	= pchip->chip_spare_size + 64;
	else
		spare_size	= pchip->chip_spare_size;
    size = page_boundary - addr + spare_size;
...
//        printf("nand_flash_write_buf(): Attempt to write past block boundary, blk 0x%x, address 0x%x, length 0x%x %x %x\n", blk, addr, len, pchip->chip_block_size, pchip->chip_spare_size);
       // return (ret - len);
//    }

    do
    {
    	//printf("blk = %d %x %x %x %x\n", blk, page_addr, page_offset, buffer, size);
        if(raw_nandflash_write_page(pchip, page_addr, page_offset, buffer, size) != FLASH_API_OK)
        {
            ret = ret - len;
            break;
        }
        ...
        if( len )
        {
            ...
            page_offset = 0;
            buffer += size;
            if(len > pchip->chip_page_size + spare_size)
                size = pchip->chip_page_size + spare_size;
            else
                size = len;
        }
    } while(len);
    ...
    unsigned int page_offset, unsigned char *buffer, int len)
{
    ...
    unsigned char xfer_buf[pchip->chip_page_size + spare_size];
    int ret = FLASH_API_ERROR;
...
//printf("%x %x %x %x %x\n", xfer_buf, page_offset, len, pchip->chip_spare_step_size, pchip->chip_spare_step_size);
        nandflash_copy_to_spare(xfer_buf + pchip->chip_page_size + len * pchip->chip_spare_step_size, pchip->chip_spare_step_size);
