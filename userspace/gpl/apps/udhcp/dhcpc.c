...
#ifdef MSTC_DHCP_121
#define DHCP_OPTION_121_NUM 32
extern struct dhcp_option121 option121[32];
#endif
...
void setStatus(int status)
{
   ...
   else
   {
      ...
      if (wasAssigned == 1)
      {
#ifdef MLD_GLOB_TTNET_FEATURE //_MSTC_, Linda, DHCP Log
      	if(state != RELEASED)
      	{
      		//NACK
      		...
      		if((fp = fopen("/var/dhcp_server", "r")) != NULL){
      			while (fgets(fileIP, sizeof(fileIP), fp))
      			{
      				...
      				if(cmsUtl_strcmp(file_row[0], inet_ntoa(temp_addr))==0)
      				{
      					log_len += sprintf(message_log, "DHCPNACK src mac=%s dst mac=%02x:%02x:%02x:%02x:%02x:%02x src ip=%s", file_row[1],
      					...
      					zyLog_app_save2flash(LOG_LEVEL_NOTICE, LOG_FAC_DHCPCLIENT, message_log);
      					break;	
      				}
      			}
      			fclose(fp);
      		...
      		{
      			log_len += sprintf(message_log, "DHCPNACK src mac=ff:ff:ff:ff:ff:ff dst mac=%02x:%02x:%02x:%02x:%02x:%02x src ip=%s",
      				client_config.arp[0], client_config.arp[1], client_config.arp[2],client_config.arp[3], client_config.arp[4], client_config.arp[5], inet_ntoa(temp_addr));
      			log_len += sprintf(message_log+log_len, "dst ip=%s ", inet_ntoa(temp2_addr));
      			sprintf(message_log+log_len, "assigned ip=%s", inet_ntoa(temp2_addr));
      			zyLog_app_save2flash(LOG_LEVEL_NOTICE, LOG_FAC_DHCPCLIENT, message_log);
...
static void release_requested(int sig)
{
	...
	if (state == BOUND || state == RENEWING || state == REBINDING) {
		...
		if((fp = fopen("/var/dhcp_server", "r")) != NULL){
			...
			{
				sscanf(fileIP, "%s %s", file_row[0], file_row[1]);
				if(cmsUtl_strcmp(file_row[0], inet_ntoa(tmp_addr))==0)
				{
					log_len += sprintf(message_log, "DHCPRELEASE src mac=%02x:%02x:%02x:%02x:%02x:%02x dst mac=%s src ip=%s ", client_config.arp[0], client_config.arp[1], 
		...
		{
			log_len += sprintf(message_log, "DHCPRELEASE src mac=%02x:%02x:%02x:%02x:%02x:%02x dst mac=ff:ff:ff:ff:ff:ff src ip=%s ", client_config.arp[0], client_config.arp[1], 
...
/* Merge option 121 from DSL-491HNU-B1B_V2_TTNET: r2991 */
	...
	char RouteRule[256];
#endif
#ifdef MLD_GLOB_TT_HOME_GW_FEATURE	/*__MSTC__, Linda, DHCP LOG add Interface Name and VLAN. */
	FILE *fp;
	char tmp_mac[BUFLEN_64] = {0};
	char tmp_intfName[BUFLEN_64] = {0};
	SINT32 tmp_vlan = 0;
	char tempbuf[BUFLEN_512] = {0};
	char WANMAC[BUFLEN_64]={0};
	char intfName[BUFLEN_64]={0};
	SINT32 vlan = 0;
#endif	/*__MSTC__, Linda, DHCP LOG add Interface Name and VLAN_END. */
...
*/
		c = getopt_long(argc, argv, "c:fH:hi:np:qr:s:d:a:u:o:P:m:S:v:6V:", options, &option_index);
#else
		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
...
#endif
				{
...
#ifdef MLD_GLOB_TT_HOME_GW_FEATURE	/*__MSTC__, Linda, DHCP LOG add Interface Name and VLAN. */
					snprintf(WANMAC, sizeof(WANMAC)-1, "%02x:%02x:%02x:%02x:%02x:%02x", client_config.arp[0], client_config.arp[1], client_config.arp[2],client_config.arp[3], client_config.arp[4], client_config.arp[5]);
						
					if((fp = fopen("/var/vlan_info", "r")) != NULL){
						while(fscanf(fp, "%[^\n]\n", tempbuf) > 0){
							if (sscanf(tempbuf, "%s %s %d", tmp_mac, tmp_intfName, &tmp_vlan ) == 3 ){
								if(cmsUtl_strcasecmp(tmp_mac, WANMAC) == 0){
									cmsUtl_strncpy(intfName, tmp_intfName, sizeof(intfName)-1);
					...
					if( strlen(intfName) != 0){
						if(vlan == -1){
							zyLog_app_save2flash(LOG_LEVEL_NOTICE, LOG_FAC_DHCPCLIENT, "DHCPDISCOVERY src mac=%02x:%02x:%02x:%02x:%02x:%02x dst mac=ff:ff:ff:ff:ff:ff src ip=0.0.0.0 dst ip=255.255.255.255 interface=%s vlan=N/A",
								client_config.arp[0], client_config.arp[1], client_config.arp[2],client_config.arp[3], client_config.arp[4], client_config.arp[5], intfName);
						}
						else{
							zyLog_app_save2flash(LOG_LEVEL_NOTICE, LOG_FAC_DHCPCLIENT, "DHCPDISCOVERY src mac=%02x:%02x:%02x:%02x:%02x:%02x dst mac=ff:ff:ff:ff:ff:ff src ip=0.0.0.0 dst ip=255.255.255.255 interface=%s vlan=%d",
								client_config.arp[0], client_config.arp[1], client_config.arp[2],client_config.arp[3], client_config.arp[4], client_config.arp[5], intfName , vlan);
						}
					}
					else
						zyLog_app_save2flash(LOG_LEVEL_NOTICE, LOG_FAC_DHCPCLIENT, "DHCPDISCOVERY src mac=%02x:%02x:%02x:%02x:%02x:%02x dst mac=ff:ff:ff:ff:ff:ff src ip=0.0.0.0 dst ip=255.255.255.255",client_config.arp[0], client_config.arp[1], client_config.arp[2],client_config.arp[3], client_config.arp[4], client_config.arp[5]);						
#else
					zyLog_app(LOG_LEVEL_NOTICE, LOG_FAC_DHCPCLIENT, "DHCPDISCOVERY src mac=%02x:%02x:%02x:%02x:%02x:%02x dst mac=ff:ff:ff:ff:ff:ff src ip=0.0.0.0 dst ip=255.255.255.255",client_config.arp[0], client_config.arp[1], client_config.arp[2],client_config.arp[3], client_config.arp[4], client_config.arp[5]);
#endif
#endif
...
#ifdef MSTC_LOG //__MSTC__,Lynn,Log
				  temp2_addr.s_addr=requested_ip;
#ifndef MLD_GLOB_TTNET_FEATURE //_MSTC_, Linda, DHCP Log
				  zyLog_app_save2flash(LOG_LEVEL_NOTICE, LOG_FAC_DHCPCLIENT, "Send DHCP request to %s", inet_ntoa(temp_addr));
				  zyLog_app_save2flash(LOG_LEVEL_NOTICE, LOG_FAC_DHCPCLIENT, "DHCPREQUEST src mac=%02x:%02x:%02x:%02x:%02x:%02x dst mac=ff:ff:ff:ff:ff:ff src ip=0.0.0.0 dst ip=255.255.255.255 requested ip=%s",
				  	client_config.arp[0], client_config.arp[1], client_config.arp[2],client_config.arp[3], client_config.arp[4], client_config.arp[5],inet_ntoa(temp2_addr));
#endif
#endif
				} else {
...
#ifdef MLD_GLOB_TT_HOME_GW_AAWV_FEATURE //__MSTC__, Bruce
					send_release(server_addr, requested_ip);
#endif
					/* timed out, enter init state */
					state = INIT_SELECTING;
					LOG(LOG_INFO, "Lease lost, entering init state");
#if !defined(MLD_GLOB_TTNET_FEATURE) || defined(MLD_GLOB_TT_HOME_GW_AAWV_FEATURE)					
					run_script(NULL, "deconfig");
#endif
					timeout = time(0);
					packet_num = 0;
...
#ifndef MLD_GLOB_TTNET_FEATURE //_MSTC_, Linda, DHCP Log
						zyLog_app_save2flash(LOG_LEVEL_NOTICE, LOG_FAC_DHCPCLIENT, "Receive DHCP offer from %s", inet_ntoa(temp_addr));
#endif
#endif
...
/* Merge option 121 from DSL-491HNU-B1B_V2_TTNET: r2991 */
#ifdef MSTC_DHCP_121
					//__MSTC__, Hill, Tracking Table item214 for STB routing rule issue
					int op121found = 0;
					FILE* option121file;
					char filename[128]= {0};

					sprintf(filename,"/var/option121RouteRule_%s",client_config.interface);
					// option 121: add to routing table after bound, sherry
					option121file = fopen(filename,"a");
					for(i=0; i<5; i++){
						if((strlen(option121[i].option121_target) == 0) || (strlen(option121[i].option121_gateway) == 0))
							break;
					...
					//__MSTC__, Hill, Tracking Table item214 for STB routing rule issue
						...
						sprintf(cmd, "route add -net %s netmask %s gw %s dev %s", option121[i].option121_target,
						option121[i].option121_mask, option121[i].option121_gateway, client_config.interface);	
						printf("%s\n", cmd);
						system(cmd);
						if(option121file == NULL){
							printf("\n  open %s fail !!\n",filename);
						}else{
					            sprintf(RouteRule,"%s \n",cmd);
   					            fputs(RouteRule,option121file);
						}
					}
					if(option121file != NULL){
						fclose(option121file);	
					}
#endif						
