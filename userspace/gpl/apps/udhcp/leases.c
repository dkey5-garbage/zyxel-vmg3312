...
#ifdef MLD_GLOB_TTNET_FEATURE /*Marge HostName issue from DSL-491HNU-B1B_V2_TTNET (ticket#68, #125, #282, #371)*/
void adjust_lease_time(unsigned long  delta)
#else
...
/*Marge HostName issue from DSL-491HNU-B1B_V2_TTNET (ticket#68, #125, #282, #371)*/
	...
	for (cur_iface = iface_config ; cur_iface ; cur_iface = cur_iface->next)
	{
		for (cur_arp = arp_head ; cur_arp != NULL ; cur_arp = cur_arp->next)
		{
			...
			if(subnet_option != NULL){ //__MSTC__, work around for second bridge with dhcp relay will be crash(dhpcd)
				ptr = subnet_option->data;
				subnet_mask = (*(ptr + 2) & 0xff) << 24 | (*(ptr + 3) & 0xff) << 16 | (*(ptr + 4) & 0xff) << 8 | (*(ptr + 5) & 0xff);

				if ((cur_arp->yiaddr & subnet_mask) == (temp_iface->server & subnet_mask))
				{
					if (staticHost)
					{
						...
						else
						{
							...
							if( (cur_arp->yiaddr >= cur_iface->start) && (cur_arp->yiaddr <=cur_iface->end))
							{
								...
								send_lease_info(FALSE, staticHost, FALSE);
#else
					...
					else
					{
						...
						&& (cur_arp->yiaddr >= cur_iface->start) && (cur_arp->yiaddr <=cur_iface->end)) 
						{
							add_lease_MSTC(cur_arp->chaddr, cur_arp->yiaddr, server_config.offer_time, TRUE);
							staticHost = find_lease_by_chaddr(cur_arp->chaddr);
							strcpy(staticHost->hostname, "unknown");
...
void handleHostInfoUpdate(CmsMsgHeader *msg)
{
   ...
   if ((yiaddr & subnet_mask) == (cur_iface->server & subnet_mask))
   {
      if (leaseHost)
      {
         ...
         {
#ifdef MLD_GLOB_TTNET_FEATURE /*Marge HostName issue from DSL-491HNU-B1B_V2_TTNET (ticket#68, #125, #282, #371)*/
	         add_lease_MSTC(chaddr, yiaddr, body->leaseTimeRemaining, TRUE);
#else
