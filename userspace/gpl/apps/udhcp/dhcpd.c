...
#ifndef MLD_GLOB_TTNET_FEATURE //__MSTC__, Steven: TR111 can't work
...
/*Marge HostName issue from DSL-491HNU-B1B_V2_TTNET (ticket#68, #125, #282, #371)*/
#ifdef MLD_GLOB_TTNET_FEATURE
               if (msg->dataLength == sizeof(unsigned long)) 
#else    
               if (msg->dataLength == sizeof(long)) 
#endif
               {
#ifdef MLD_GLOB_TTNET_FEATURE
                  unsigned long  *delta = (unsigned long  *) (msg + 1); 
#else
...
/* __MSTC__, klose, QoS classifier rule DHCP option exclude support, 20121207 */
		...
		if (cur_iface->relay_interface[0]) {
			...
			int add_bytes = 0;
			char copy_mac[BUFLEN_16]={0};   //Client MAC Address.
			sprintf(copy_mac,"%02x:%02x:%02x:%02x:%02x:%02x",packet.chaddr[0],packet.chaddr[1],packet.chaddr[2],packet.chaddr[3],packet.chaddr[4],packet.chaddr[5]);
			char AGinfo[AGENT_INFO_LEN];
			//Add option 82 in packet.
			if (createAGoption(AGinfo, copy_mac) != -1)
			{
				add_bytes = add_option_string(packet.options, (unsigned char *)AGinfo);
			}
			bytes += add_bytes;
#endif
...
//						fprintf(stderr, "host name: %s\n", tmpHostName);
					...
					if (server_id) {
						...
						if (server_id_align == cur_iface->server && requested && 
							...
							if(vsi != NULL)
								send_lease_info(FALSE, lease, TRUE);
							else
								send_lease_info(FALSE, lease, FALSE);
#else
							send_lease_info(FALSE, lease);
...
#ifdef MLD_GLOB_TTNET_FEATURE	/* __MSTC__, Linda, OPTION 125 means it is STB. */
				send_lease_info(TRUE, lease, FALSE);
#else
