...
#ifdef MSTC_DHCP_121
	/* additions for static routes */
       {"sroutes",	OPTION_STATIC_ROUTES | OPTION_REQ, 0x79}, /* sroutes */
       {"sroutes",	OPTION_STATIC_ROUTES | OPTION_REQ, 0xF9}, /* cidrroute for win */    // sherry option 249
#endif
	{"",		0x00,				0x00}
};
...
/* Merge option 121 from DSL-491HNU-B1B_V2_TTNET: r2991 */
#ifdef MSTC_DHCP_121
	,[OPTION_STATIC_ROUTES] = 1 /* it has no limited length */
#endif
};
...
CmsRet queryDeviceInfo(GetDeviceInfoMsgBody *pDeviceInfo)
{
   ...
   else
   {
      ...
      if ((ret = cmsMsg_receiveWithTimeout(msgHandle, &msg, 5*MSECS_IN_SEC)) != CMSRET_SUCCESS)
#else
...
/* OPTION 125 */
        ...
        printf("getBaseMacAddress:%02X:%02X:%02X:%02X:%02X:%02X", macAddr[0],  macAddr[1],  macAddr[2],  macAddr[3], macAddr[4],  macAddr[5]);
#endif	
...
/* OPTION 125 */
#ifdef MLD_GLOB_TTNET_FEATURE
        sprintf(deviceSerialNum, "%02X%02X%02X%02X%02X%02X", macAddr[0],  macAddr[1],  macAddr[2],  macAddr[3], macAddr[4],  macAddr[5]);	
#else
...
#ifdef MSTC_DHCP_43
CmsRet queryOption43Info(GetURLOption43InfoMsgBody *urlOption43Info)
{
   CmsRet ret = CMSRET_SUCCESS;
   void *re_msgBuf;
   void *msgBuf[sizeof(CmsMsgHeader) + sizeof(GetURLOption43InfoMsgBody)] ={0};
   CmsMsgHeader *msg = (CmsMsgHeader *)msgBuf;
   GetURLOption43InfoMsgBody *body = (GetURLOption43InfoMsgBody*) (msg+1);
   
   msg->type = CMS_MSG_GET_URLOPTION43_INFO;
   msg->src = EID_DHCPD;
   msg->dst = EID_SSK;
   msg->flags_request = 1;
   msg->dataLength = sizeof(GetURLOption43InfoMsgBody);
   ...
   else
   {
      ...
      else
      {
         switch(msg->type)
         {
         case CMS_MSG_GET_URLOPTION43_INFO:
            if(msg->dataLength != sizeof(GetURLOption43InfoMsgBody))
            {
               cmsLog_error("Invalid Data Length: received %d != expected %d",
                            msg->dataLength, sizeof(GetRelayOption82InfoMsgBody));
               
               ret = CMSRET_INTERNAL_ERROR;
               break;
            }
            re_msgBuf = (GetURLOption43InfoMsgBody *)(msg + 1);
            memcpy(urlOption43Info,re_msgBuf,sizeof(GetURLOption43InfoMsgBody));
            break;
              
         default:
            cmsLog_error("Invalid message type (%x), expects CMS_MSG_GET_URLOPTION43_INFO (%x)", (unsigned int)msg->type,CMS_MSG_GET_URLOPTION43_INFO);
            ret = CMSRET_INTERNAL_ERROR;
         }
...
int createUrloption43(char *option43info)	/* OPTION 43 */
{
	char optionData[128], *dataPtr;
	int len, totalLen = 0;

	GetURLOption43InfoMsgBody UrlOption43Info;
	if (queryOption43Info(&UrlOption43Info) != CMSRET_SUCCESS)
	{
		return -1;
	}
		
	/* optionData[0] = Suboption. */
	optionData[DHCP_OPTION_43_CODE_OFFSET] = (char)DHCP_OPTION_43_CODE;

	dataPtr = optionData + DHCP_OPTION_43_DATA_OFFSET;
	
	/* Suboption data. */
	*dataPtr++ = (char)DHCP_OPTION_43_LEN_OFFSET;

	len = strlen(UrlOption43Info.url) ;  // Length = "url"
	*dataPtr++ = len;
	strncpy(dataPtr, UrlOption43Info.url, strlen(UrlOption43Info.url));

	totalLen += (len + DHCP_OPTION_43_SUBCODE_AND_LEN_BYTES);


	/* optionData[1] = totalLen. */
	optionData[DHCP_OPTION_43_LEN_OFFSET] = totalLen;

	/* also copy the option code and option len which is not counted in total len */
	memcpy((void*)option43info,(void*)optionData,(totalLen+DHCP_OPTION_43_DATA_OFFSET));
	return 0;
}
#endif
#ifdef MSTC_DHCP_82	//_MSTC_, YingYu, Option 82
/* Send message to ssk for get some information of Relay option 82. */
CmsRet queryAssociateInfo(GetRelayOption82InfoMsgBody *pRelayOption82Info, char *chaddrptr)
{
   CmsRet ret = CMSRET_SUCCESS;
   void *re_msgBuf;
   void *msgBuf[sizeof(CmsMsgHeader) + sizeof(GetRelayOption82InfoMsgBody)] ={0};
   CmsMsgHeader *msg = (CmsMsgHeader *)msgBuf;
   GetRelayOption82InfoMsgBody *body = (GetRelayOption82InfoMsgBody*) (msg+1);
   
   msg->type = CMS_MSG_GET_RELAYOPTION82_INFO;
   msg->src = EID_DHCPD;
   msg->dst = EID_SSK;
   msg->flags_request = 1;
   msg->dataLength = sizeof(GetRelayOption82InfoMsgBody);
   
   strncpy(body->mac,chaddrptr,BUFLEN_32);
   ...
   else
   {
      ...
      else
      {
         switch(msg->type)
         {
         case CMS_MSG_GET_RELAYOPTION82_INFO:
            if(msg->dataLength != sizeof(GetRelayOption82InfoMsgBody))
            {
               cmsLog_error("Invalid Data Length: received %d != expected %d",
                            msg->dataLength, sizeof(GetRelayOption82InfoMsgBody));
	     ...
	     re_msgBuf = (GetRelayOption82InfoMsgBody *)(msg + 1);
            memcpy(pRelayOption82Info,re_msgBuf,sizeof(GetRelayOption82InfoMsgBody));
            break;
              
         default:
            cmsLog_error("Invalid message type (%x), expects CMS_MSG_GET_RELAYOPTION82_INFO (%x)", (unsigned int)msg->type,CMS_MSG_GET_RELAYOPTION82_INFO);
            ret = CMSRET_INTERNAL_ERROR;
         }
...
/* Establish the information of option 82 in packet. */
int createAGoption(char *AGinfo, char *chaddrptr)	/* OPTION 82 */
{
	char optionData[AGENT_INFO_LEN], *dataPtr;
	int len, totalLen = 0;

	GetRelayOption82InfoMsgBody RelayOption82Info;
	if (queryAssociateInfo(&RelayOption82Info, chaddrptr) != CMSRET_SUCCESS)
	{
		return -1;
	}
	...
	len = strlen(RelayOption82Info.productClass) + strlen(RelayOption82Info.SSID) + 3;  // Length = "productClass" + "SSID" + 2*space + Security.
     	*dataPtr++ = len;
 	strncpy(dataPtr, RelayOption82Info.productClass, strlen(RelayOption82Info.productClass));
	dataPtr += strlen(RelayOption82Info.productClass);
	*dataPtr++ = (char)AGENT_SPACE;  //space
	strncpy(dataPtr, RelayOption82Info.SSID, strlen(RelayOption82Info.SSID));
	dataPtr += strlen(RelayOption82Info.SSID);
	*dataPtr++ = (char)AGENT_SPACE; //space
	if(RelayOption82Info.NoSecurity == 0)
		*dataPtr++ = (char)AGENT_NOSECURITY;
	else
		*dataPtr++ = (char)AGENT_SECURITY;
     	totalLen += (len + AGENT_SUBCODE_AND_LEN_BYTES);

   	 //REMOTE_ID = First CPE MACAddress
   	char mac_address_value[16]="";
   	UINT8 macAddr[6] = {0};
	devCtl_getBaseMacAddress(macAddr);
   	snprintf(mac_address_value, sizeof(mac_address_value),"%02X%02X%02X%02X%02X%02X", macAddr[0],  macAddr[1],  macAddr[2],  macAddr[3], macAddr[4],  macAddr[5]);
	
        *dataPtr++ = (char)AGENT_REMOTE_ID;
  	len = strlen(mac_address_value);
     	*dataPtr++ = len;
  	strncpy(dataPtr, mac_address_value, len);
     	dataPtr += len;
    	totalLen += (len + AGENT_SUBCODE_AND_LEN_BYTES);
