...
static void release_iface_config(struct iface_config_t * iface)
{
	...
	source_port_t *port_cur,*port_next;
#endif
	...
	if (iface->vendor_id_mode){
		free(iface->vendor_id_mode);
	}
	mac_cur = iface->macs;
	while(mac_cur) {
	...
	while(vsi_cur) {
        ...
        iface->vsi = NULL;
	port_cur = iface->source_port;
	while(port_cur) {
		port_next = port_cur->next;
		if (port_cur->source_port) {
			free(port_cur->source_port);
        	}
		free(port_cur);
		port_cur = port_next;
	}
	iface->source_port=NULL;		
#endif
#ifdef MSTC_STATIC_DHCP_AUTO //__MSTC__, Lynn, Static DHCP automatically for STB
	vid_cur = iface->opt60ForSTB;
	while(vid_cur) {
		vid_next = vid_cur->next;
		if (vid_cur->id) {
			free(vid_cur->id);
	...
	while (fgets(buffer, 80, in)) {
		...
		*line = '\0';
		...
		if (strcasecmp(token, "interface") == 0) {
			...
			char poolDelim[] = "_";
			char *poolInterface = strdup(cur_iface->interface);
			char *poolSocketIf;
			if( strstr(cur_iface->interface, "condservpool") ){
				...
				cur_iface->UsedIf = strdup(poolSocketIf);
			}else{
				/*It is physical interface*/
				cur_iface->UsedIf = strdup(cur_iface->interface);
			}
			free(poolInterface);
#endif
...
#ifdef MLD_GLOB_TTNET_FEATURE	/* __MSTC__, Linda, OPTION 125 means it is STB. */
	cmsLog_debug("isSTB=%d, isOPT125=%d", isSTB, isOPT125);
	if((!isSTB) && (isOPT125 == TRUE))
	{
		snprintf(body->hostType, sizeof(body->hostType), MDMVS_SETTOP);
		isSTB = TRUE;
	}
...
/* for ttnet,if wifi client go through repeater to connect CPE, then the interface type will show other. this is a workaround solution.*/
   else if (strstr(intfName, ""))
      snprintf(body->interfaceType, sizeof(body->interfaceType), MDMVS_802_11);
#endif
   else
