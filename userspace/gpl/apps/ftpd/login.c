...
CmsRet queryUserInfo(GetUserInfoMsgBody *GetUserInfo)
{
   void *msgBuf[sizeof(CmsMsgHeader) + sizeof(GetUserInfoMsgBody)] ={0};
   CmsMsgHeader *msg = (CmsMsgHeader *)msgBuf;
   GetUserInfoMsgBody *body = (GetUserInfoMsgBody*) (msg+1);
   void *re_msgBuf;
   CmsRet ret = CMSRET_SUCCESS;
   
   msg->type = CMS_MSG_FTP_USER_INFO;
   msg->src = EID_FTPD;
   msg->dst = EID_SSK;
   msg->flags_request = 1;
   msg->dataLength = sizeof(GetUserInfoMsgBody);

   strncpy(body->username,userinfo.pw_name, 257);
   strncpy(body->password,userinfo.pw_passwd, 257);
   ...
   else
   {
      ...
      else
      {
         switch(msg->type)
         {
         	case CMS_MSG_FTP_USER_INFO:
         		if(msg->dataLength != sizeof(GetUserInfoMsgBody))
         		{
         			cmsLog_error("Invalid Data Length: received %d != expected %d", msg->dataLength, sizeof(GetUserInfoMsgBody));               
         	...
         	default:
         		cmsLog_error("Invalid message type (%x), expects CMS_MSG_FTP_USER_INFO (%x)", (unsigned int)msg->type,CMS_MSG_FTP_USER_INFO);
         		ret = CMSRET_INTERNAL_ERROR;
         }
      } 
   }
...
int UserAllow() 
{
	...
	if (queryUserInfo(&GetUserInfo) != CMSRET_SUCCESS)
		return -1;
...
//printf("In bftpd_login password=%s, user=%s\n", password, user); // brcm
	...
	if (checkpass(password))
   {
    ...
    if (UserAllow() == -1)
    {
    	cmsLog_debug("This user is not allow to log in.");
    	return 2;
    }
#endif	/*_MSTC_, Linda, CPE User Definitions and Rights_END.*/
#if 0 //brcm
