...
//printf("In command_pass password=%s\n", password); // brcm
	...
	SINT32 ReturnLoginValue = bftpd_login(password);
	if ( ReturnLoginValue == 2) {
...
//__MSTC__, Nick Tseng
   ...
   else
   {
      ...
      cmsImg_writeValidatedImage(imagePtr, uploadSize, imageFormat, msgHandle);  /*_MSTC_,YingYu, For TTNET download. */
#ifdef MSTC_ROM_D //__COMMON__, jchang, ROM-D 
...
#ifdef MSTC_ROM_D //__COMMON__, jchang
...
#if 0 /*ZyXEL Jacky, rename backupsettings.conf to rom-0*/
...
#if 0 /*ZyXEL Jacky, support image updates when FTPD storage feature is defined */
...
//__MSTC__, Nick Tseng, GUI Debug Mode with FTP
	...
	FILE *imageFilepPtr = NULL;
	CmsImageFormat glbImgFormat;
	CmsRet ret = CMSRET_INVALID_IMAGE;
	char filepath[256] = {0};
	unsigned long pos;
	unsigned long F_len;
	FILE *dbg_fp = NULL;
    dbg_fp = fopen("/tmp/ftpddbglog.txt", "a+");
	if (!strcmp(parentDir, "/firmware/fw/")) {
		sprintf(filepath, "%s%s", parentDir, filename);
	      	imageFilepPtr = fopen(filepath, "r+");
              if( imageFilepPtr == NULL ) {				  	
			  	control_printf(SL_SUCCESS, "Open file fail.");
			  	cmsLog_error("Failed to reopen file %s.", mapped);
		} else {	
			/* Size calculation of the transfered file. */
			pos = ftell(imageFilepPtr);
			F_len = 0;
			fseek(imageFilepPtr, 0L, SEEK_END);
			F_len = ftell (imageFilepPtr);
			fseek(imageFilepPtr, pos, SEEK_SET);
			
			glbImgFormat = cmsImg_validateImageFile(imageFilepPtr, (int) F_len, msgHandle);	
			if (glbImgFormat != CMS_IMAGE_FORMAT_BROADCOM){	
				control_printf(SL_FAILURE, "Invalid Image!!\n");	
				fclose(imageFilepPtr);	
				fclose(dbg_fp);
				return;	
			} else {	
				control_printf(SL_SUCCESS, "Open file success and transfer size is %d\nTrying to flash the image file...\n", (int)F_len);	
				ret = cmsImg_writeValidatedImageFile(imageFilepPtr, (int) F_len, glbImgFormat, msgHandle, filepath);
	 			if (ret != CMSRET_SUCCESS) {
					control_printf(SL_FAILURE, "Image Write Failed!\n");
					fclose(imageFilepPtr);
                    fclose(dbg_fp);
				return;
				}
			}
			control_printf(SL_SUCCESS, "Image Write Successed!\n");
		}
		if(imageFilepPtr) {
		fclose(imageFilepPtr);		
			imageFilepPtr = NULL;
...
#if 0 /*ZyXEL Jacky, support config backup when FTPD storage feature is defined */
