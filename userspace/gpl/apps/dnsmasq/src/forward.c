...
static struct query_domain *create_domain_rec(char *namebuff,unsigned short QType)
{
  ...
  if (!flags && forward)
    {
      ...
      struct query_domain *n ;
      int found = 0;
	//my_syslog(LOG_INFO, _("namebuff ,%s,"),daemon->namebuff);

		for(n = daemon->domainQuery; n ; n=n->next)
		{
			if(!strncmp(n->domainname,daemon->namebuff,MAXDNAME) && n->QType == gotname)//richard
			{
				found = 1;
				break;
			}
		...
		if (found){
			...
			if(diff >= 30 || diff <= -30 )
			{
				n->timer = now_time;
				n->queryCount = 0;
				//my_syslog(LOG_INFO, _("namebuff ,%s, domainname %s, queryCount %d"),daemon->namebuff, n->domainname, n->queryCount);
			}
		}else{
			n = create_domain_rec(daemon->namebuff,gotname);//richard
...
//my_syslog(LOG_INFO, _("namebuff ,%s, domainname %s, queryCount %d"),daemon->namebuff, n->domainname, n->queryCount);
		if(strstr(n->domainname, "tr069.ttnet.com.tr") != NULL && n->queryCount ==2)
			return 1;
...
#if 1 /*CDRouter-ipv6_dns_45, CDRouter-ipv6_dns_46*/ 
