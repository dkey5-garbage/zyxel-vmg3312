...
void queue_process(int force, unsigned long testFileLength) {
#else
...
#if 1 //__MSTC__,SUPPORT TR143 FTP UPLOAD TEST
				if(testFileLength >0){
				res = fsession_transmit_file(F, opt.curftp,testFileLength);
				}else{
				res = fsession_transmit_file(F, opt.curftp,0);
				}
#else
