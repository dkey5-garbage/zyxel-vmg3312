...
#if 1 //__MSTC__,SUPPORT TR143 FTP UPLOAD TEST
int do_send_143(_fsession * fsession, unsigned long testFileLength){
#define DBUFSIZE 1024
	char        databuf[DBUFSIZE];
	int         fd          = open_input_file(fsession);
	int         readbytes   = 0;
	...
	unsigned char backupbarstyle = opt.barstyle;
       char buf[1400]={0};
       unsigned long len=testFileLength;
	struct wput_timer * timers[2];
	...
	if(interfaceCatch){
	    catchIfTXStatistics(ifName, &ifBytesTXStart);
	}
	while(len>0 ) {    
		...
		if(opt.speed_limit > 0 ) {
			...
			if(len > 1400){
			    ...
			    transfered_size += 1400;
			    transfered_last += 1400;
			    res = socket_write(fsession->ftp->datasock, buf, 1400);
                         len-=1400;
			    if(res != 1400) {
				    printout(vLESS, _("Error: "));
				    printout(vLESS, _("Error encountered during uploading data (%s)\n"), strerror(errno));
			    ...
			    memset(buf,'F',sizeof(len));
			    transfered_size += len;
			    transfered_last += len;
			    res = socket_write(fsession->ftp->datasock, buf, len);
			    if(res != len) {
				    printout(vLESS, _("Error: "));
				    printout(vLESS, _("Error encountered during uploading data (%s)\n"), strerror(errno));
				    free(timers[0]);
...
#if 1 //__MSTC__,SUPPORT TR143 FTP UPLOAD TEST
int fsession_transmit_file(_fsession * fsession, ftp_con * ftp, unsigned long testFileLength) {
#else
int fsession_transmit_file(_fsession * fsession, ftp_con * ftp) {
...
#if 1 //__MSTC__,SUPPORT TR143 FTP UPLOAD TEST
   if(testFileLength >0){	
       fsession->local_fsize = testFileLength;
   }
#endif
  while(!fsession->done && ( fsession->retry > 0 || fsession->retry == -1) ){
    ...
    if (testFileLength ==0){	
        ...
        while((res = do_send_143(fsession, testFileLength)) == ERR_RETRY) {
            retry_wait(fsession);
            if(!( fsession->retry > 0 || fsession->retry == -1)) {
                res = ERR_FAILED;
                break;
