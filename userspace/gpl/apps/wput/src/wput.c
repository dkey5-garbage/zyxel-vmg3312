...
#if 1 //__MSTC__,SUPPORT TR143 FTP UPLOAD TEST
#include <time.h>
#include "cms_msg.h"
#include "cms_util.h"
#include "cms_log.h"
...
#if 1 //__MSTC__,SUPPORT TR143 FTP UPLOAD TEST
extern char diagnosticsState[64];
#if 1//__QWEST__,JhenYang
extern char tcpOpenReqTime[64];
#else
static char tcpOpenReqTime[64];
#endif
#if 1 //__COMMON__, Dennis
#define DEFAULT_DATETIME "0.0 0001-01-01T00:00:00Z"
#endif
extern char tcpOpenResTime[64];
extern char dateTimeBufROM[64];
extern char dateTimeBufBOM[64];
extern char dateTimeBufEOM[64];
unsigned long testFileLength = 0;
unsigned long totalBytesSent = 0;
int requestEid=0;
#if defined(MSTC_ONLY_TTNET_FEATURE) || defined(MLD_GLOB_TT_HOME_GW_FEATURE)/* _MSTC_, Linda, Syslog Upload. */
char *filename = NULL;
#endif
#if 1	/* DSCP & EthernetPriority for upload diagnostic profile in TR143, __TELEFONICA__, ZyXEL Eva, 20100112.  */
int dscpMark = 0;
#if 0//__QWEST__,JhenYang
int priorityMark = PRIORITYDEFAULT;
#else
int priorityMark = 0x1000;
#endif
#endif
static void *msgHandle = NULL;
#if 1//__QWEST__,JhenYang
#define MAXSESSIONNUM 10
#define UPLOADD_LOCK_TIMEOUT (3*MSECS_IN_SEC)
int multiID=0;
extern unsigned long ifBytesTXStart;
unsigned long ifBytesTXEnd = 0;
extern int interfaceCatch;
extern char ifName[32];
extern void getXSIDateTime(unsigned long t, char *buf, unsigned long bufLen);
extern void createTR143UploadInfo(int sessionID);
extern void catchIfTXStatistics(char *ifName, int *bytes);
#endif
#endif
...
#if 1
#if 1//__QWEST__,JhenYang
void createTR143UploadInfo(int sessionID)
#else
void createTR143UploadInfo()
#endif
{
	char line[1518];
	FILE* fs = NULL;
	...
	sprintf(cmd, "/var/tr143_upload.info_%d", sessionID);
	fs = fopen(cmd, "w");
#else
	fs = fopen("/var/tr143_upload.info", "w");
#endif
	if ( fs != NULL ) 
	{
		...
		fputs(line, fs);
		sprintf(line, "testFileLength:%ld\n", testFileLength);
		fputs(line, fs);
		sprintf(line, "totalBytesSent:%ld\n", totalBytesSent);
		fputs(line, fs);
		...
		fputs(line, fs);
#if 1//__QWEST__,JhenYang
		sprintf(line, "interfaceTXStart:%ld\n", ifBytesTXStart);
		fputs(line, fs);		
		sprintf(line, "interfaceTXEnd:%ld\n", ifBytesTXEnd);
		fputs(line, fs);
#endif	
		fclose(fs);
   	}
...
#if 1//__QWEST__,JhenYang
void catchIfTXStatistics(char *ifName, int *bytes)
{
		FILE *fp;
		int read;
		int txBytes;
		...
		while (fgets(buf, sizeof(buf), fp) != NULL) {
			...
			if ((ptr = strchr(buf, ':')) == NULL ||
				...
				printout(vNORMAL,"Wrong format of /proc/net/dev. Sorry.");
				return -1;
			}
			
			if (sscanf(ptr, "%*ld%*ld%*ld%*ld%*ld%*ld%*ld%*ld%ld%*[^\n]", &txBytes) != 1)
				continue;
			
			if(strstr(interfaceName, ifName)){
				fprintf(stderr, "interfaceName:%s, TXBytes:%ld\n", interfaceName, txBytes);
				*bytes = txBytes;
				break;
			}
		}
...
#  endif
...
#endif	
#ifdef MSTC_ONLY_TTNET_FEATURE		/* _MSTC_, Linda, Syslog Upload. */
		g_sysupload_err  = 0;
#endif
#if defined(MSTC_ONLY_TTNET_FEATURE) || defined(MLD_GLOB_TT_HOME_GW_FEATURE)

	g_packetcaptureupload_err  = 0; /* _MSTC_, Rex, PacketCapture Upload. */
#endif
	opt.sbuf      = malloc(82);
	opt.retry     = -1;
...
#if 1 //__MSTC__,SUPPORT TR143 FTP UPLOAD TEST
	queue_process(0,testFileLength);
#endif
...
#if 1 //__MSTC__,SUPPORT TR143 FTP UPLOAD TEST
			       if(testFileLength ==0){
				    res = fsession_transmit_file(fsession_queue_entry_point, opt.curftp,0);
			       }else{
				    res = fsession_transmit_file(fsession_queue_entry_point, opt.curftp,testFileLength);
			       }
#else
...
#ifdef MSTC_ONLY_TTNET_FEATURE		/* _MSTC_, Linda, Syslog Upload. */
				if((res == -1)||(res == -2)||(res == -3)||(res == -4)||(res == -8)||(res == -9))
				{
					g_sysupload_err  = 1;
				}
#endif
	...
	if(testFileLength >0){
	    ...
	    if (interfaceCatch){
               catchIfTXStatistics(ifName, &ifBytesTXEnd);
	    }
	    strcpy(diagnosticsState, "Completed");
	    fprintf(stderr, "EOMTime:%s\r\n", dateTimeBufEOM);
	    fprintf(stderr, "diagnosticsState:%s\r\n", diagnosticsState);
	    createTR143UploadInfo(0);
	    cmsMsg_init(EID_WPUT, &msgHandle);
	    if(requestEid==EID_TR69C){
	        sendEventMessage(CMS_MSG_UPLOAD_DIAG_COMPLETE_EVENT);
	    }
#if 1	/* For upload diagnostic profile in TR143, __TELEFONICA__, ZyXEL Eva, 20091120.  */
	    char EndOfSendTime[64];
	    getXSIDateTime(0, EndOfSendTime, sizeof(EndOfSendTime));

	    fprintf(stderr, "\n------------ This timestamp is just for debugging ------------\r\n");
	    fprintf(stderr, "EndOfSendTime(The last packet sent out from CPE interface):\r\n");
	    fprintf(stderr, "%s\r\n", EndOfSendTime);
	    fprintf(stderr, "--------------------------------------------------------------\r\n\n");
#endif	
	}
	...
	if(opt.transfered > 0)
	{
		if( (strcmp(filename, "/log/system.log") == 0) && (requestEid == EID_TR69C))
		{
			cmsMsg_init(EID_WPUT, &msgHandle);
			sendEventMessage(CMS_MSG_SYSLOG_UPLOAD_COMPLETE_EVENT);
		}
	}
	if( (g_sysupload_err == 1) && (strcmp(filename, "/log/system.log") == 0) && (requestEid == EID_TR69C)) 
	{
		cmsMsg_init(EID_WPUT, &msgHandle);
		sendEventMessage(CMS_MSG_SYSLOG_UPLOAD_FAIL_EVENT);
	}
#endif
#if defined(MSTC_ONLY_TTNET_FEATURE) || defined(MLD_GLOB_TT_HOME_GW_FEATURE)

	/* _MSTC_, Rex, PacketCapture Upload. */
	if(opt.transfered > 0)
	{
		if( (strstr(filename, "/firmware/") != NULL || strstr(filename, "/mnt/") != NULL) && (requestEid == EID_TR69C))
		{
			cmsMsg_init(EID_WPUT, &msgHandle);
			sendEventMessage(CMS_MSG_PACKETCAPTURE_UPLOAD_COMPLETE_EVENT);
		}
	}
	if( (g_packetcaptureupload_err == 1) && ((strstr(filename, "/firmware/") != NULL || strstr(filename, "/mnt/") != NULL)) && (requestEid == EID_TR69C)) 
	{
		cmsMsg_init(EID_WPUT, &msgHandle);
		sendEventMessage(CMS_MSG_PACKETCAPTURE_UPLOAD_FAIL_EVENT);
	}
#endif
	if(opt.transfered > 0)
...
void commandlineoptions(int argc, char * argv[]){
    ...
    {
#if 1 //__MSTC__,SUPPORT TR143 FTP UPLOAD TEST
        c = getopt_long (argc, argv, "Vhbo:a:dqvn:i:I:t:NT:w:Rl:pABsS:u:z",
#else
