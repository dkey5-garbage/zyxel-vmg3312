...
#define FILE_VOIP_IS_IN_USE	"/var/voip_is_in_use"
int isVoiceInUse(){
	FILE * fp = NULL;
	int value = 0;
	int ret = 0;

	fp = fopen(FILE_VOIP_IS_IN_USE,"r");
	if(fp){
		fscanf(fp,"%d",&value);
		fclose(fp);
	}else{
...
int isLogFileNeedSave(const char *fileName){
	int ret = 0;
	if(!strncmp(fileName,LOG_FILE_NEED_SAVE,strlen(LOG_FILE_NEED_SAVE))){
		ret = 1;
	}else if(!strncmp(fileName,SECURITY_LOG_FILE_NEED_SAVE,strlen(SECURITY_LOG_FILE_NEED_SAVE))){
		ret = 1;
	}else if(!strncmp(fileName,BROADCOM_LOG_FILE_NEED_SAVE,strlen(BROADCOM_LOG_FILE_NEED_SAVE))){
		ret = 1;
	}
...
#if ENABLE_FEATURE_ROTATE_LOGFILE
#ifdef MSTC_SAVE_LOG_TO_FLASH //__MSTC__, TengChang
		if (G.logFileSize && G.isRegular && G.curFileSize > (G.logFileSize - 1024)&& isLogFileNeedSave(G.logFilePath) && !isVoiceInUse()) {		
			if (G.logFileRotate) {
#ifdef SYSLOGD_WRLOCK
				fl.l_type = F_UNLCK;
				fcntl (G.logFD, F_SETLKW, &fl);
			...
			if(!isLogFileNeedSave(G.logFilePath)){/*log won't save to flash, just delete*/
				if(G.logFileSize && G.isRegular && G.curFileSize > (G.logFileSize - 1024)){
					memset(cmd,0,sizeof(cmd));
					snprintf(cmd,sizeof(cmd),"rm -f %s",G.logFilePath);
					system(cmd);
				}
			}else{
				if(G.logFileSize && G.isRegular && G.curFileSize > ((G.logFileSize - 2048)*2)){
					/*voice is in use,log want to save, but log size to large, just delete*/
					memset(cmd,0,sizeof(cmd));
					snprintf(cmd,sizeof(cmd),"rm -f %s",G.logFilePath);
					system(cmd);
				}
...
static void timestamp_and_log(int pri, const char *msg, int len)
{
   ...
   int logType = SYSTEM_TYPE;
	...
	if(!strncmp(str,"NeedSave2Flash: ",strlen("NeedSave2Flash: "))){
		is_log_need_save = 1;
		str+=strlen("NeedSave2Flash: ");
	}
...
*/
	...
	if(securityLog){
		...
		if(is_log_need_save){
			G.logFilePath = SECURITY_LOG_FILE_NEED_SAVE;
		}else{
	...
	if(brcmLog){
		...
		tempFlashLogFilePath = flashLogFilePath;
		if(is_log_need_save){
			G.logFilePath = BROADCOM_LOG_FILE_NEED_SAVE;
		}else{
	...
	if(strncmp(G.logFilePath,LOG_FILE,strlen(LOG_FILE)) == 0){
		if(is_log_need_save){
			G.logFilePath = LOG_FILE_NEED_SAVE;
		}
	}
...
static void syslogd_ip_mac(char *input1, int size1, char *input2, int size2, const struct in_addr *serverIP)
{
...
#ifdef MLD_GLOB_TTNET_FEATURE//__MSTC__, Cinmex, Add Eth0 MAC in syslog for TTNET
	FILE *fp;
	char cmd[32];
	char *macpath = "/var/mac";
	char tmp[64];
	...
	if (strcmp(logIp, "\0")) {
		...
		if (fp2 != NULL) {
			if (fscanf(fp2, "%*[^\n]\n") < 0)
			{
				fclose(fp2);	//__MSTC__, Hill, [B10A #365] When syslog setting is Local File and Remote, DeviceLog did not overwrite old logs
				return;
			}
			while (1) {
				int r;
				...
				if ((syslog_server_addr & m) == d) {
					...
					if ((ioctl(sockfd, SIOCGIFHWADDR, &ifr)) >= 0) {
#ifdef MLD_GLOB_TTNET_FEATURE//__MSTC__, Cinmex, Add Eth0 MAC in syslog for TTNET
						snprintf(cmd, 64, "ifconfig %s | grep HWaddr > %s", ifname, macpath);
						system(cmd);
						...
						HWaddr = strstr(tmp,"HWaddr");
						memcpy(tmp_mac_Address,HWaddr+7, 17);
						sprintf(mac_Address,"%s", tmp_mac_Address);
						mac_Address[17] = '\0';
						system("rm -r /var/mac");
						snprintf(input2, sizeof(mac_Address), mac_Address);
						fclose(fp);	//__MSTC__, Hill, [B10A #365] When syslog setting is Local File and Remote, DeviceLog did not overwrite old logs
#else
					...
					fclose(fp2);
					close(sockfd);	////__MSTC__, Hill, [B10A #365] When syslog setting is Local File and Remote, DeviceLog did not overwrite old logs
					return;
				}
			}
			...
			if ( !msg->flags_response ) {
				if(msg->type == CMS_MSG_CLEAR_LOG){
					...
					if(OPT_locallog && !(G.shbuf))
					{
						...
						checkIsLogFull(flashLogFilePath);
#ifdef MSTC_SYS_AND_SEC_LOG
						copyLogToFlash(SECURITY_LOG_FILE_NEED_SAVE, SECURITY_FLASH_LOG_FILE, G.logFileSize, G.logFileRotate);
						checkIsLogFull(SECURITY_FLASH_LOG_FILE);
#endif
#ifdef MSTC_HIDE_BRCM_LOG
						copyLogToFlash(BROADCOM_LOG_FILE_NEED_SAVE, FLASH_BROADCOM_LOG_FILE, G.logFileSize, G.logFileRotate);
#endif
