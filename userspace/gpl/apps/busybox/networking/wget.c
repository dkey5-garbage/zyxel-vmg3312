...
unsigned long  testBytesReceived;
...
static void progress_meter(int flag)
{
	...
	testBytesReceived = (G.beg_range + G.transferred);
...
static FILE* prepare_ftp_session(FILE **dfpp, struct host_info *target, len_and_sockaddr *lsa, int session)
{
	char buf[512];
	FILE *sfp;
	char *str;
 ...
 pasv_error:
	...
	*dfpp = open_socket(lsa);
	...
	//Merge pipeline connections from DSL-491HNU-B1B_V2_TTNET
	if (session > 1) {
		sprintf(buf, "REST %"OFF_FMT"u", G.content_len - G.content_len/session+1);
...
int wget_main(int argc UNUSED_PARAM, char **argv)
{
	...
	if (!(opt & WGET_OPT_OUTNAME)) {
		...
		{
		    //Merge pipeline connections from DSL-491HNU-B1B_V2_TTNET
			//DownloadDiagnstic do not require file storage on th CPE device
...
#endif
	//Merge pipeline connections from DSL-491HNU-B1B_V2_TTNET
	if (( G.content_len == 0) && (sNum > 1)){
		fprintf(sfp, "HEAD /%s HTTP/1.1\r\n", target.path); //get file size
	}
	else
	{
		/* Send HTTP request */
		...
		if ( G.content_len > 0){
			if(multiID != (sNum-1)){
				fprintf(sfp, "Range: bytes=%llu-%llu\r\n",(multiID * (G.content_len/sNum)), ((multiID+1) * (G.content_len/sNum))-1);				
			}
			else{
				fprintf(sfp, "Range: bytes=%llu-\r\n",(multiID * (G.content_len/sNum)));				
			}
...
//			bb_error_msg_and_die("bad redirection (no Location: header from server)");
		 ...
		 *  FTP session
		 */
		sfp = prepare_ftp_session(&dfp, &target, lsa,sNum); //Merge pipeline connections from DSL-491HNU-B1B_V2_TTNET
	}
#if 1 // __MSTC__, Justin, Support TR143:DownloadDiagnstic
   ...
   else //support WGET_HTTPS
   {
	...
	if(sNum == 1){
		...
		fprintf(stdout, "200 OK, File Get Success(%d)\n",multiID);
	}
...
void createTR143Info(int sessionID)
{
	...
	if ( fs != NULL ) 
	{
		...
		fputs(line, fs);
		sprintf(line, "testBytesReceived:%ld\n", testBytesReceived);
		fputs(line, fs);		
