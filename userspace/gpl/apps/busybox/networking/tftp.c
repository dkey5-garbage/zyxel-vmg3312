...
// brcm end
	...
	IF_GETPUT(opt =) getopt32(argv,
			IF_FEATURE_TFTP_GET("g") IF_FEATURE_TFTP_PUT("p")
				"l:r:f:t:e:v" IF_FEATURE_TFTP_BLOCKSIZE("b:"),
			&local_file, &remote_file, &brcm_remote_file, &upload_type, &requestEid, &brcm_loglevel
...
// brcm end
	...
	char *tmp = NULL;
	if( (requestEid!=NULL) && (atoi(requestEid)==EID_TR69C) && ((tmp = strstr(local_file, "system_")) != NULL))
	{
		if( strstr(tmp, ".log") != NULL)
		{
			if(result == 0)
	...
	if( (requestEid!=NULL) && (atoi(requestEid)==EID_TR69C) && ((strstr(local_file, ".ext") != NULL || strstr(local_file, ".pcap") != NULL)))
	{
		if(result == 0)
