...
typedef enum 
{
...
#ifdef MSTC_IGMPDIAG
   CMS_MSG_IGMP_DIAG                            = 0x1000027D, /**< IGMP_DIAG completed, or stopped */
#endif
...
#ifdef MSTC_IGMPDIAG
   CMS_MSG_MCPD_DIA   	                        = 0x1000027B, /**< Sent to mcpd to  diagnosis the IGMP Status*/
#endif
...
#ifdef MSTC_AUTO_FW_UPGRADE  //__TTNET__, REX
   CMS_MSG_CHECK_NOTIFY_INFO                      = 0x1000315,
#endif
   ...
   CMS_MSG_CHANG_APP                            = 0x1000080E, /**< request smd to change an app port */
    ...
    CMS_MSG_UPLOAD_DIAG_COMPLETE_EVENT        = 0x10000813, /**< Upload dignostic completed */
#endif
#endif
   ...
   CMS_MSG_3G_IP_STATE_UNCONFIG                 = 0x10002522, /* set 3G Con->connectionStatus UNCONFIGURED */
   ...
   CMS_MSG_4G_IP_STATE                          = 0x10002523, /* get 4G ip state */
...
#if 1		//_MSTC_,YingYu, Periodic inform time. 
   CMS_MSG_WAN_CONNECTION_UP_ForTR069                    = 0x1000FF04, /**< WAN connection is up (got IP address) */
#endif

#ifdef MSTC_DHCP_82	//_MSTC_, YingYu, Option 82			
   CMS_MSG_GET_RELAYOPTION82_INFO				= 0x1000FF05,
#endif
   CMS_MSG_SAVE_CONFIG_TOFLASH                               = 0x1000FF06,
#if defined(MSTC_TTNET_NO_LIMIT) && defined(MSTC_TTNET_MOBILE_APP)
   CMS_MSG_SET_WLAN_MOBILEAPP                   = 0x1000FF07,
#endif

#ifdef MSTC_ONLY_TTNET_FEATURE
   CMS_MSG_SYSLOG_TIMEOUT_EDIT                   = 0x1000FF08,	/* _MSTC_, Linda, Syslog Timeout. */
   CMS_MSG_SYSLOG_UPLOAD_COMPLETE_EVENT                   = 0x1000FF09,	/* _MSTC_, Linda, Syslog Upload. */
   CMS_MSG_SYSLOG_UPLOAD_FAIL_EVENT                   = 0x1000FF0A,	/* _MSTC_, Linda, Syslog Upload. */
#endif

#if defined(MSTC_ONLY_TTNET_FEATURE) || defined(MLD_GLOB_TT_HOME_GW_FEATURE)
   CMS_MSG_PACKETCAPTURE_TIMEOUT                 = 0x1000FF10, /* _MSTC_, REX, PacketCapture Timeout. */
   CMS_MSG_PACKETCAPTURE_UPLOAD_COMPLETE_EVENT   = 0x1000FF11, /* _MSTC_, REX, PacketCapture Upload. */
   CMS_MSG_PACKETCAPTURE_UPLOAD_FAIL_EVENT 	  = 0x1000FF12, /* _MSTC_, REX, PacketCapture Upload. */
#endif

#ifdef SUPPORT_MSTC_MOBILE_APP // support MSTC mobile app
   CMS_MSG_MOBILEAPP_WLAN_UPDATE				= 0x1000FF0B,
   CMS_MSG_MOBILEAPP_ALERT_MSG 			 = 0x1000FF0C,
   CMS_MSG_MOBILEAPP_HOST_INFO 			 = 0x1000FF0D,
#endif

#ifdef MSTC_ONLY_TTNET_FEATURE	/* _MSTC_, Linda, Port Mirroring. */
   CMS_MSG_PORTMIRROR_TIMEOUT_EDIT                   = 0x1000FF0F,
#endif	/* _MSTC_, Linda, Port Mirroring_END. */
#ifdef MSTC_DHCP_43
   CMS_MSG_GET_URLOPTION43_INFO            = 0x1000FF13,          
#endif
#ifdef MSTC_TTNET_MULTI_USER	 /*__MSTC__, Linda, CPE User Definitions and Rights. */
   CMS_MSG_FTP_USER_INFO				 = 0x1000FF0E,
#endif
#ifdef MLD_GLOB_TT_HOME_GW_FEATURE
   CMS_MSG_UDP_CONN_REQ                 = 0x1000FF0F,
   CMS_MSG_ADD_TR69_CR_SOCK_MARK             = 0x1000FF14,      /*_MSTC_,Peter add TR69 connection request listen sock mark. */
#endif
} CmsMsgType;

#ifdef MSTC_IGMPDIAG
/** Data body for CMS_MSG_IGMP_DIAG message type.
 *
 */
typedef struct igmpdiagdatamsgbody
{
   char diagnosticsState[BUFLEN_32];
   char interface[BUFLEN_32];
   char channelAddress[BUFLEN_32];
   char channelStatus[BUFLEN_32];
   char streamServerAddress[BUFLEN_32];
   SINT32 numberOfIGMPJoins;
   SINT32 numberOfIGMPLeaves;
   SINT32 IGMPJoinDelay;
   SINT32 timeout;
...
typedef struct
{
   ...
   char serverid[BUFLEN_32];	/**< DHCP server ip, if addressAssigned==TRUE */
...
typedef struct
{
   char url[BUFLEN_128];       /**< URL of ACS */
} GetURLOption43InfoMsgBody;
#endif
#ifdef MSTC_DHCP_82	//_MSTC_, YingYu, Option 82	
/*
 * Data body for CMS_MSG_GET_RELAYOPTION82_INFO message type.
 */
typedef struct
{
   char mac[BUFLEN_32];       /**< MAC Address of LAN PC */
   char SSID[BUFLEN_32];	/**< SSID of device */
   char productClass[BUFLEN_32];    /**< product class of device */
   UBOOL8 NoSecurity;
} GetRelayOption82InfoMsgBody;
#endif
...
{
   SINT32 domain;
   char type[16]; //event type: WANUP, LANCHANGE
  
} Set_iptablesBody;
#endif

#ifdef MSTC_ONLY_TTNET_FEATURE    /* _MSTC_, Linda, New syslog parameters. */
typedef struct
{
   SINT32 timeout;  
} Set_syslogBody;
/* _MSTC_, Linda, New syslog parameters_END. */

/* _MSTC_, Linda, Port Mirroring. */
typedef struct
{
   SINT32 timeout;
} Set_PortMirrorBody;
/* _MSTC_, Linda, Port Mirroring_END. */
