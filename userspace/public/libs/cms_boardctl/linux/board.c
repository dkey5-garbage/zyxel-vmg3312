        ...
        { 
           ...
           if (ret == CMSRET_SUCCESS)
           {
              if ((boardIoctl == BOARD_IOCTL_GET_PSI_SIZE) ||
                  ...
                  (boardIoctl == BOARD_IOCTL_GET_GPIO) ||
#endif
...
#if 1//MitraStar, Julian, GPIO
int sysGetGPIO( int numGPIO, int* result )
{
	CmsRet ret = CMSRET_SUCCESS;
	
	ret = devCtl_boardIoctl(BOARD_IOCTL_GET_GPIO, 0, NULL, numGPIO, 0, result);
   	return ret;
}
int sysSetGPIO( int numGPIO, int value, int* result )
{
	CmsRet ret = CMSRET_SUCCESS;
	
	ret = devCtl_boardIoctl(BOARD_IOCTL_SET_GPIO, 0, NULL, numGPIO, value, result);
   	return ret;
}
#endif
#if 1 //__MSTC__, Dennis
