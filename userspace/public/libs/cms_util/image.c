...
static CmsRet flashImageFlash(const FILE *imageFileFp, UINT32 imageLen, CmsImageFormat format, char* filepath)
{
	...
	CmsRet ret;
#ifdef MSTC_OBM_IMAGE_DEFAULT  /* Image Default, __CHT__, MitraStar Curtis, 20111124. */
...
#endif
	{
...
#ifdef MSTC_OBM_IMAGE_DEFAULT  /* Image Default, MitraStar Curtis, 20111124. */
		...
		* was present in the legacy code, but is not used. */
		...
		printf("fwup_flash_parm.size = %d,fwup_flash_parm.offset = %d \n", fwup_flash_parm.size, fwup_flash_parm.offset);
...
CmsRet cmsImg_writeValidatedImageFile(FILE *imageFileFp, UINT32 imageLen, CmsImageFormat format, void *msgHandle, char* filepath)
{
	...
	CmsMsgHeader msg = EMPTY_MSG_HEADER;
       ...
       /* ---save config to flash first--- */
	msg.dst = EID_SSK;
	msg.src = cmsMsg_getHandleEid(msgHandle);
	msg.type = CMS_MSG_SAVE_CONFIG_TOFLASH;
	msg.flags_request = 1;
	if ( (ret = cmsMsg_send(msgHandle, &msg)) != CMSRET_SUCCESS )
	{
		  cmsLog_error( "Could not send CMS_MSG_SAVE_CONFIG_TOFLASH msg to ssk, ret=%d", ret );
	}
	usleep(500000);
	if(imageFileFp == NULL) {
		cmsLog_error("File point is NULL.");
	...
	if ((imageLen > sizeof(FILE_TAG)) && (verifyBroadcomFileTagFlash(imageFileFp) == CMSRET_SUCCESS)) {
		...
		*/
		...
		if (imageLen > TOKEN_LEN)
		{
			...
			buf = fread(buf, 1, TOKEN_LEN, imageFileFp);
			crcPtr = buf;
			/*
