...
#if 1 /***Merge DNS feature from TT.***/
	else
	{
		if(cmsUtl_strcmp(tmpBuf,"0.0.0.0") == 0){
			strcpy(outDnsSecondary, "01.01.01.01");//means CPU disable DNS
		}
	}
#endif
...
UBOOL8 cmsUtl_isUnsafeString(const char *compareChar)
{
   ...
   for (i = 0 ; i < data_len ; i++)
   {
      for (j = 0 ; j < sizeof(unsafeString) ; j++)
      {
...
UBOOL8 cmsUtl_isUnsafeStringForWlKey(const char *compareChar)
{
   SINT32 i, j, data_len;
   char unsafeString[] = {'\"', '<', '>', '%', '\\', '^', '[', ']', 
                              '`', '+', '$', '=', '\'', '#', '&', '\t', 
