                      ...
                      u_int8_t pf, unsigned int hooknum)
{
   ...
   //DEBUGP("%s (0x%x) %pI4 <-> %pI4 status %s info %d %s\n", __FUNCTION__, 
   //   esphdr->spi, &iph->saddr, &iph->daddr, (ct->status & IPS_SEEN_REPLY) ? "SEEN" : "NOT_SEEN", ctinfo, (ctinfo == IP_CT_NEW ) ? "CT_NEW" : "SEEN_REPLY" );
                   ...
                   unsigned int dataoff)
{
   ...
   //DEBUGP("%s SPI(0x%x) %pI4 <-> %pI4 ct %p\n",
   //        __FUNCTION__, esphdr->spi, &iph->saddr, &iph->daddr, ct);
   if (esphdr != NULL)
   {
