...
extern char AGinfo[142];
extern int add_option82_len ;
#define DHCP_MARK		0x63825330	//for mark match packet
#define DHCP_MAGIC		0x63825363	//Magic cookie: DHCP
...
#if 1//ENABLE_FEATURE_UDHCP_RFC3397
...
int add_option_string(unsigned char *optionptr, unsigned char *string)
{
	...
	printk( "\n\r adding option 0x%02x", string[OPT_CODE]);
...
#if 1 //for add DHCP option82
	struct iphdr _iph, *ih=NULL;
	struct tcpudphdr_t _ports, *pptr=NULL;
	struct bootp_pkt *_bootp=NULL,*bootp=NULL;
	char *skb_p = NULL;
	unsigned int len_option = 0;
	int add_bytes = 0;
	int z=0 , j =0, x=0;
		...
		if (EBT_MATCH_ITERATE(point, ebt_do_match, skb, &mtpar) != 0)
		 {
			...
			if (bootp->cookie != htonl(DHCP_MARK)){
				//this is not match packet
				goto fail_print;
			}
			else{
				bootp->cookie = htonl(DHCP_MAGIC);
				
				len_option = (unsigned int)((skb->tail)-(bootp->exten));
				skb_p = skb_put(skb, AGinfo[1]+2);
				add_bytes = add_option_string(bootp->exten,  (unsigned char *)AGinfo);

				j= end_option(bootp->exten)+1;
				z = len_option-(j-AGinfo[1]-2);
				for(x=0;x<z; x++)
				{
				     bootp->exten[j+x]= DHCP_PADDING;
				}

				ih->tot_len += (AGinfo[1]+2);
				...
				skb->csum = csum_partial(bootp, ntohs(pptr->len) - sizeof(struct tcpudphdr_t), 0);
				pptr->check = 0;
                pptr->check = csum_tcpudp_magic(ih->saddr, ih->daddr, ntohs(ih->tot_len) - (ih->ihl << 2),
                ih->protocol, csum_partial(pptr, sizeof(struct tcpudphdr_t), skb->csum));			
...
static int translate_table(char *name, struct ebt_table_info *newinfo)
{
	...
	/* we now know the following (along with E=mcB2):
