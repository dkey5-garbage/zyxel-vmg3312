...
#if !defined(CONFIG_11ac_throughput_patch_from_412L07)||defined(CONFIG_MSTC_SUPPORT_TTNET)
                       ...
                       uint32_t param1, uint32_t param2 )
{
    ...
    switch ( request )
    {
	    ...
	    /* __MSTC__, Hill, CSP#828274 patch */
            if( (((unsigned int )(dev_p->put_stats)) & 0x80000003) != 0x80000000 )
            {
               printk("%s:%d Wrong function pointer ! dev_p:%p dev_p->put_stats:%p\n",__FUNCTION__,__LINE__,dev_p,dev_p->put_stats);
               return 0;
            }
