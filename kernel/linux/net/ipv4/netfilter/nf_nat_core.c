		 ...
		 enum nf_nat_manip_type maniptype)
{
	...
	*tuple = *orig_tuple;
	...
	/*MSTC_WX,StevenGao,Fix the bug that default SIP port is used by ATA so that CPE cannot register SIP server successfully*/
#ifdef CONFIG_MSTC_SUPPORT_TTNET_HGW
	if(tuple->src.u.all == nf_sip_reg_port){
		printk("proto->unique_tuple3,tuple->src.u.all=%d\n",tuple->src.u.all);
		proto->unique_tuple(tuple, range, maniptype, ct);
		goto out;
	}
#endif
