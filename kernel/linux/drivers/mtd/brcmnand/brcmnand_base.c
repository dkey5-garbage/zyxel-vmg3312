...
print_oobbuf(p8, 16);
		...
		u_char uncErr_Data[512];
		erased = 1;
		allFF = 1; // Not sure for BCH.   modify this line.
		memset(uncErr_Data,0,sizeof(uncErr_Data));
		brcmnand_from_flash_memcpy32(chip, uncErr_Data, offset, ECCSIZE(mtd));
        for (i = 0; i < ECCSIZE(mtd); i++) {
            if(uncErr_Data[i] !=0xff) 
            {
                allFF=0;
                break;
            }
