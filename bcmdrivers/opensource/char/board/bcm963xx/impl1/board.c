...
/*After restoredefault 0,doesn't write config to flash,__CHT__,MSTC Peien,20130912*/
#define RESTORE_FILE "/var/ignoreSave"
...
int chkWlanBtnTimer;  // __MSTC__, Hill, add for TTNET WLAN Button
struct timer_list gWlanTimer; // __MSTC__, Hill, add for TTNET WLAN Button
#else
...
uint8 stopblink = 0;
uint8 stopblinkwl = 0;
uint8 stopblinkwps = 0;	//__MSTC__, Hill, add for TTNET WLAN Button
uint8 buttonTest = 0;
...
#if defined (WIRELESS) /*&& !defined(MSTC_WPS_BTN_SCE)*/ // __MSTC__, Paul Ho, for WLAN Button	//__MSTC__, Hill, mark MSTC_WPS_BTN_SCE define for TTNET
void WlanTimerExpire(void)
{
   ...
   else
   {
      if ( chkWlanBtnTimer >= CV_WLAN_BTN_PERIOD*10 ) {
         printk(KERN_WARNING "\r\n WLAN Behavior %d\n", chkWlanBtnTimer);
         if ( buttonTest == 1 ) {	//__MSTC__, Hill, for TTNET
            if ( stopblinkwl == 0 ) {
            ...
            printk(KERN_WARNING "\r\n WLAN Button is Pressed, send SIGUSR1 to wlmngr pid_no:%d\n", pid_no);
      ...
      else if ( chkWlanBtnTimer >0 && buttonTest == 1 )	//__MSTC__, Hill, for TTNET
      {
         printk(KERN_ERR "\r\n WLAN Button is Pressed\n");
         if ( stopblinkwl == 0 ) {
            //kerSysLedCtrl(kLedPowerG, kLedStateFastBlinkContinues);
            stopblinkwl = 1;
         }
...
#if defined (WIRELESS) /*&& !defined(MSTC_WPS_BTN_SCE)*/ // __MSTC__, Paul Ho, for WLAN Button	//__MSTC__, Hill, mark MSTC_WPS_BTN_SCE for TTNET
...
printk("flashFsKernelImageFile !!!!!!!!!!!\n");
	...
	printk("rootfsAddr = %u\n", rootfsAddr);
	printk("kernelAddr = %u \n", kernelAddr);
	printk("rootfsOffset is %d \n", rootfsOffset);
	printk("newImgSize is %d \n", newImgSize);

#ifdef MSTC_OBM_IMAGE_DEFAULT //__MSTC__, Dennis ZyXEL OBM ImageDefault feature, zongyue
	if (0x1 == pTag->imageNext[0]) {
		UINT32 search_tag_addr = 0;
		search_tag_addr = newImgSize;
		printk("search_tag_addr = %8.8lx\n", search_tag_addr);
	...
	kerSysFlashAddrInfoGet(&flash_info);
	printk("rootfsOffset = %u , flash_info.flash_rootfs_start_offset = %lu\n", rootfsOffset, flash_info.flash_rootfs_start_offset);
	if( rootfsOffset < flash_info.flash_rootfs_start_offset ) {
		// Increase rootfs and kernel addresses by the difference between
		// rootfs offset and what it needs to be.
		rootfsAddr += flash_info.flash_rootfs_start_offset - rootfsOffset;
	...
	rootfsAddr += BOOT_OFFSET;
	kernelAddr += BOOT_OFFSET;
	printk("rootfsAddr = 0x%x\n", rootfsAddr);
	printk("kernelAddr = 0x%x \n", kernelAddr);
	#if 1
	availableSizeOneImg = flash_info.flash_rootfs_max_size;
	availableSizeTwoImgs = flash_info.flash_rootfs_max_size;
	printk("availableSizeOneImg = %u, availableSizeTwoImgs = %u\n", availableSizeOneImg, availableSizeTwoImgs);
	#else
	reservedBytesAtEnd = flash_get_reserved_bytes_at_end(&flash_info);
	...
	printk("curImgSize = %u", curImgSize);
	printk("newImgSize is %u \n", newImgSize);
    // If the current image fits in half the flash space and the new
    // image to flash also fits in half the flash space, then flash it
    // in the partition that is not currently being used to boot from.
	if( curImgSize <= availableSizeTwoImgs &&
...
#if 1
            /*After restoredefault 0,doesn't write config to flash,__CHT__,MSTC Peien,20130912*/
            fp = filp_open(RESTORE_FILE, O_RDONLY, 0);
            if (!IS_ERR(fp))
            {
                filp_close(fp, NULL);
                /*file /var/ignoreSave exist, skip save config to flash*/
                printk("\n...skip save config to flash...\n");
                return 0;
            }
#endif
...
//               ret = kerSysBcmImageSet(ctrlParms.offset, ctrlParms.string, ctrlParms.strLen);
    ...
    case BOARD_IOCTL_SET_GPIO:
        ...
        break;
#if 1//MitraStar, Julian, GPIO
     case BOARD_IOCTL_GET_GPIO:	
            if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
//            kerSysGetGpioState(ctrlParms.strLen, ctrlParms.offset);
                ctrlParms.result = kerSysGetGpioState(ctrlParms.strLen);
                __copy_to_user((BOARD_IOCTL_PARMS*)arg, &ctrlParms, sizeof(BOARD_IOCTL_PARMS));
                ret = 0;
            } 
            else {
   ...
   case BOARD_IOCTL_SET_BUTTONTEST:
      if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
         if ( ctrlParms.strLen == 1 ) {
            ...
            stopblink = 0;
            stopblinkwl = 0;
            stopblinkwps = 0;	//__MSTC__, Hill, for TTNET
         ...
         }else if ( ctrlParms.strLen == 5 ) {	//__MSTC__, Hill, for TTNET
            ctrlParms.result = stopblinkwps;
         }
         __copy_to_user((BOARD_IOCTL_PARMS*)arg, &ctrlParms, sizeof(BOARD_IOCTL_PARMS));
         ret = 0;
      } 
   ...
   struct task_struct *p;
	if ( sesBtn_pressed() ) {
		...
		if (chkWlanEnCounter%10 == 0 && chkWlanEnCounter < 200) // do not display "." continuously, just within 20 sec (avoid to affect some other boards)
			printk(".");
		// start timer
#ifdef MSTC_WPS_BTN_PER
		if ( chkWlanEnCounter == MSTC_WPS_BTN_PER*10 )
		{
			if ( buttonTest == 0 ) {	//__MSTC__, Hill, for TTNET
				//struct task_struct *p;
...
#ifdef MSTC_WLAN_BTN_PER
		//if ( chkWlanEnCounter >= MSTC_WLAN_BTN_PER*10 )
#endif /* MSTC_WLAN_BTN_PER */
/*		{
			printk("\n");
...
#ifdef MSTC_WLAN_BTN_PER
		if ( chkWlanBtnCounter >= MSTC_WLAN_BTN_PER*10 )
#endif /* MSTC_WLAN_BTN_PER */
		{
			printk("\n");
			printk(KERN_WARNING "WLAN toogle behavior\n");
...
static GPIO_STATE_t GetGpio(int gpio)
{
	#if defined(CONFIG_BCM96362) || defined(CONFIG_BCM963268)	//__MSTC__, Hill, for TTNET
...
#if (defined(CONFIG_BCM963268) || defined(CONFIG_BCM_6362_PORTS_INT_EXT_SW)) && (CONFIG_BCM_EXT_SWITCH > 0)
