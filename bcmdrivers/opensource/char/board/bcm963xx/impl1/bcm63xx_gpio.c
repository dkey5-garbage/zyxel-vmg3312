...
GPIO_STATE_t kerSysGetGpioState(int gpio)
{
    unsigned long flags = 0;
    GPIO_STATE_t ret = kGpioInactive;

    spin_lock_irqsave(&bcm_gpio_spinlock, flags);

    BCM_ASSERT_V(gpio != BP_NOT_DEFINED);
    BCM_ASSERT_V((gpio & BP_GPIO_NUM_MASK) < GPIO_NUM_MAX);
    BCM_ASSERT_HAS_SPINLOCK_V(&bcm_gpio_spinlock);
...
#if 1 /* Change the Product Model according to GPIO pin value when supporting 2T2R, __CHT__, MSTC Victor, 20130104 */
EXPORT_SYMBOL(kerSysGetGpioState);
#endif
