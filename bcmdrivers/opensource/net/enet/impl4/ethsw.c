...
#else // stanley - get speed & duplex from ASR reg (0x19) -- CSP#733098
	//ethsw_phy_rreg(phyId, MII_ASR, &v16);
     if (MII_ASR_FDX(v16))
         ps.fdx = 1;
     else
...
void ethsw_switch_manage_port_power_mode(int portnumber, int power_mode)
{
   ...
   uint16 v16 = 0;
#ifdef BUILD_MSTC_DSL_2492GNAU_B1BC_MLD //__MSTC__, Eason
   int phy_id = ethsw_phyport_to_phyid(portnumber);   
   if(portnumber >= BP_MAX_SWITCH_PORTS){
   		portnumber -= BP_MAX_SWITCH_PORTS;   	
   }
   #else
   int phy_id = switch_pport_phyid[portnumber];   
   #endif
