...
#if 1
#if 0
	//__MSTC__, DelonYu, merge patch from CSP#448131
...
#else
	//Enable HP Auto-MDIX for AC201(10/100BASE-TX Transceiver)
	//external phy4 (LAN1, eth0)
	x16 = 0x002c;
	ethsw_phy_wreg(ETHSW_PHY_GET_PHYID(4), 0x1c, &x16);

	x16 = 0x008b;
	ethsw_phy_wreg(ETHSW_PHY_GET_PHYID(4), 0x1f, &x16); // Shadow Register Enable
	x16 = 0x3000;
	ethsw_phy_wreg(ETHSW_PHY_GET_PHYID(4), 0x1a, &x16); // Init. Auxiliary Mode 4
	x16 = 0x000b;
	ethsw_phy_wreg(ETHSW_PHY_GET_PHYID(4), 0x1f, &x16); // Shadow Register Disable
#endif
#if 1 //__MSTC__, DelonYu, turn on if you have to support giga LAN4
	//internal phy3 (LAN4)
	x16 = 0x0200;
...
int ethsw_get_hw_stats(int port, int extswitch, struct net_device_stats *stats) {
    ...
    if (extswitch) {
        ...
        stats->multicast = tempctr64;
        ctr64 += tempctr64;
        extsw_rreg(PAGE_MIB_P0 + (port), REG_MIB_P0_RXBPKTS, data, 4);
        tempctr64 = swab32(*(uint32 *)data);
        stats->rx_broadcast_packets = tempctr64;
        ctr64 += tempctr64;
        ...
        tempctr64 = swab32(*(uint32 *)data);
        stats->tx_multicast_packets = tempctr64;
        ctr64 += tempctr64;
        extsw_rreg(PAGE_MIB_P0 + (port), REG_MIB_P0_TXBPKTS, data, 4);
        tempctr64 = swab32(*(uint32 *)data);
        stats->tx_broadcast_packets = tempctr64;
        ctr64 += tempctr64;
