...
#ifndef BUILD_MSTC_DSL_2492GNAU_B1BC_MLD // MitraStar Elina, Reverse vport <--> phyport mapping
...
if(IsWanPort(ethsw_phyport_to_phyid(sw_port))) {
                if(speed == 1000) {
...
static void bcm63xx_enet_poll_timer(unsigned long arg)
{
    ...
    while (atomic_read(&poll_lock) > 0)
    {
            ...
            if ((priv->linkState & mask) != tmp)
            {
                ...
                if (phys.lnk)
                {
...
#endif
                    {
                        ...
                        #ifndef SUPPORT_MSTC_ETHWAN_DOWN // The Link priority is ETHWAN > DSL
                        if(strcasecmp(vnet_dev[i]->name, "eth4") ==0)
                        {
                            kerSysSendtoMonitorTask(MSG_NETLINK_BRCM_ETHNET_DELAY,NULL,0);							  
                        };
                        #endif
                    }
                }
        ...
        if (phys.lnk){
            ...
            /* Link is down, so isolate the Phy. To prevent switch rx lockup 
            because of packets entering switch with DLL/Clock disabled */
            if ((IsExtPhyId(downPhy) )&& (count ==1)) {
                ethsw_isolate_phy(downPhy, 1);
                count =0;			
            }
        }
#endif
...
#if 1 /*Eason, ethernet port mirror*/
/***************************************************************************
 * Function Name: MirrorPacket
 * Description  : This function sends a sent or received packet to a LAN port.
 *                The purpose is to allow packets sent and received on the WAN
...
#if 1 /*Eason, ethernet port mirror*/
	if( port_id_from_dev(dev) == 4 && pParam->pDevPriv->szMirrorIntfOut[0] != '\0' )
	{
		MirrorPacket(pParam->pDevPriv->szMirrorIntfOut, pNBuff);
	}
#endif	
#ifdef CONFIG_BLOG
    /*
    ...
    while (budget > 0) {
        ...
        if (rxdma->pktDmaRxInfo.rxAssignedBds != 0)
        {
            ...
            if (dev != NULL)  /* validate device */
            {
                ...
                if (pDevCtrl->szMirrorIntfIn[0] != '\0'){
                   pNBuf = FKBUFF_2_PNBUFF(pFkb);
                   MirrorPacket(pDevCtrl->szMirrorIntfIn, pNBuf);
                }
#endif
                if (global.dump_enable) {
                   DUMP_PKT(skb->data, skb->len);
...
/*For limit gpht highest speed*/
    ...
    ethsw_switch_manage_port_power_mode(3, 0);	//eth4 phy-power down
    ethsw_switch_manage_port_power_mode(0, 0);	//eth1 phy-power down
    ethsw_switch_manage_port_power_mode(1, 0);	//eth2 phy-power down
    ethsw_switch_manage_port_power_mode(2, 0);	//eth3 phy-power down

    ethsw_switch_manage_port_power_mode(3, 1);	//eth4 phy-power up
    ethsw_switch_manage_port_power_mode(0, 1);	//eth1 phy-power up
    ethsw_switch_manage_port_power_mode(1, 1);	//eth2 phy-power up
    ethsw_switch_manage_port_power_mode(2, 1);	//eth3 phy-power up
...
#endif
#ifdef BUILD_MSTC_DSL_2492GNAU_B1BC_MLD //__MSTC__, Eason
			mii->phy_id  = ethsw_phyport_to_phyid(swPort);
			//printk(KERN_ERR "%s, swPort %d, phyId 0x%x\n", __FUNCTION__, swPort, mii->phy_id);
#else
...
*/
       ...
       case SIOCETHSWCTLOPS:
        ...
        case SIOCPORTMIRROR:
            ...
            {
                //printk("\n\n ETH SIOCPORTMIRROR MIRROR_DIR_IN \n\n");
                if( mirrorCfg.nDirection == MIRROR_DIR_IN )
                {
                    if( mirrorCfg.nStatus == MIRROR_ENABLED ){
                        strcpy(pDevCtrl->szMirrorIntfIn, mirrorCfg.szMirrorInterface);
                        //printk("\n\n SIOCPORTMIRROR MIRROR_DIR_IN szMirrorIntfIn=[%s]\n\n",pDevCtrl->szMirrorIntfIn);
                    }else
                        memset(pDevCtrl->szMirrorIntfIn, 0x00, MIRROR_INTF_SIZE);
                    }
                else /* MIRROR_DIR_OUT */
                {
                    if( mirrorCfg.nStatus == MIRROR_ENABLED ){
                        strcpy(pDevCtrl->szMirrorIntfOut, mirrorCfg.szMirrorInterface);
                        //printk("\n\n SIOCPORTMIRROR MIRROR_DIR_OUT szMirrorIntfOut=[%s]\n\n",pDevCtrl->szMirrorIntfOut);
                    }else
                        memset(pDevCtrl->szMirrorIntfOut, 0x00, MIRROR_INTF_SIZE);
                    }
                }
        break;
#endif
